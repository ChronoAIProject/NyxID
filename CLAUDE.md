...
- MCP catalog fallback: a catalog-backed UserService whose catalog template has zero `ServiceEndpoint` rows falls through to the instance's user-mounted `openapi_spec_url` (custom-endpoint parse path); registered catalog rows keep precedence. Empty + no spec still drops the service from the operation catalog.
