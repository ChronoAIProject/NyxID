...
CatalogEditor service accounts use global `nyxid:catalog:skills:read/write` role permissions and matching token/live scopes for all existing and future catalog services. Admin role assignment atomically sets sticky `purpose=catalog_editor` and protection. `/keys` and `/keys/{uuid}` additionally require `user-services:read` and return projected catalog metadata with catalog UUIDs and `resource_type: "catalog_service"`; no per-resource grants or private connections. Skill writes transactionally fence live role/account/token authority. General and legacy Curation SAs retain their separate connection metadata grant behavior. See `docs/SERVICE_ACCOUNTS.md`.
