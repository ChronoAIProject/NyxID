...
- Catalog `inference` is optional admin metadata (protocol/model list/realtime). Compute caller-specific binding and `status_slug`; never store them. Startup defaults respect `inference_admin_modified` (including explicit null clears) and include admin-created `chrono-llm`/`chrono-llm-public` and seeded xAI. Provider-linked status uses `ProviderConfig.slug`; other status uses the catalog slug. See `docs/PLATFORM_KEYS_AND_INFERENCE.md` and `docs/API_DISCOVERY.md`.
- Catalog `platform_key` uses the existing encrypted master credential and live public/restricted polymorphic-owner grants. Every platform-bound resolver must call the shared ACL before decryption; no anonymous grant, no node routing or user-controlled destination. Gateway-URL providers cannot enable platform keys. Restricted ACLs batch memberships/active owners; catalog, keys (including provisioning/reconciliation), MCP and LLM listings share one request membership snapshot across service/owner checks. Reuse loaded providers or batch them once; `available_with_grants` must perform no database calls. Never cache these grants across requests. Legacy absent configs preserve public internal master-key behavior. `UserService.credential_binding` is optional; `POST/PUT /keys` and hosted/MCP flows use `use_platform_key`. Switching to platform retains the user's key. Public audience auto-provisioning is personal-only; restricted audience auto-provisioning creates personal rows for allowlisted people and org rows only for allowlisted orgs reached through active `can_proxy()` memberships. Startup removes legacy public-audience org auto rows with their orphan endpoints. Explicit org-owned platform bindings (`source != auto_provision`) retain the live public execution ACL. Scoped agent auto-connected grants include active auto rows and explicit platform bindings. Platform-key usage is always billed to the acting person, never to an org wallet, via `BillingOwnerResolver::resolve_for_execution`.
- `ServiceBilling.byok_pricing` / `platform_key_pricing` are optional credential-class lanes: the existing fields are the primary price; optional `components` add unique metrics. Either lane present supersedes legacy platform billing; missing lane and `NoAuth` are free. An unsynced primary substitutes legacy billing (or free) for the entire lane; a synced primary and synced extras charge, while unsynced extras are free and never add fallback. Resale stays independent. Primary Lago codes remain `platform_svc_{slug}_{byok|pk}`, components append `_{metric}`, with durable component cleanup/reconcile. Omitted lanes/components preserve them; null/[] clears components. Token/image capture checks every configured component independent of slug/sync; priced token classes subtract caches included in OpenAI/Gemini input, but keep Anthropic's separate cache counts; provider `TokenBreakdown` remains unchanged. Computed admin `allowance_metrics` lists configured units plus legacy only without lanes or with an unsynced primary; allowances match identical metrics. Default BYOK primary, then platform primary, then legacy. Prices support `PRICE_FRACTIONAL_DIGITS = 12`, optional pico rates plus truncated legacy micros; multiply exactly in i128, truncate gross costs to micros, ceil exact wallet remainders to whole credits. Keep funding precedence, plan charge IDs and ledger canonical encoding unchanged. Upgrade ALL replicas before authoring component prices, new-metric allowances, or prices beyond six decimals; old replicas cannot deserialize the new enum units. See `docs/PLATFORM_KEYS_AND_INFERENCE.md`.
...
- Delegated management parity requires the exact `account:read` scope, `GET`, a non-WebSocket request, and a route outside the deny classes in `mw/auth.rs`; the verified `AuthUser` extractor is authoritative. New secret-delivering, execution-shaped, streaming, upgrade, or authentication/provisioning protocol GET routes MUST be added to `delegated_read_denied_path` before mounting. Channel media downloads (`GET /api/v1/channel-relay/messages/{message_id}/attachments/{index}`) are denied; the metadata-only `GET /api/v1/channel-platforms` remains allowed for delegated `account:read`.
...
- Catalog master credentials (including legacy internal rows) always use server transport; HTTP/WS and MCP ignore owner-node bindings for these rows. This intentionally hardens legacy behavior: nodes inject only node-owned credentials. Explicit platform connections reject node selection.
...
- `ApiKey` scope fields (absorbed from the deleted `AgentGroup` model): `allowed_service_ids`, `allowed_node_ids`, `allow_all_services`, `allow_auto_connected_services`, `allow_all_nodes`; enforced at proxy time via `key_service`
...
Platform-admin service account create/update with explicit `catalog:skills:read/write` scopes atomically sets server-only `catalog_scope_authorized=true`, sticky `purpose=catalog_editor`, and protection. These accounts use matching token/live catalog scopes across all existing and future catalog services without catalog roles, activation, or per-resource grants. `/keys` and `/keys/{uuid}` need only catalog read scope and return projected catalog metadata with catalog UUIDs and `resource_type: "catalog_service"`; no private connections. Legacy editors (marker absent/false) retain live global `nyxid:catalog:skills:read/write` role checks and additional `user-services:read` for key GETs until explicit admin scope save. Stored General/Curation scope strings never auto-elevate. Skill writes transactionally fence account/token authority and legacy authorizing roles. General and legacy Curation SAs retain their separate connection metadata grant behavior. See `docs/SERVICE_ACCOUNTS.md`.
...
- General agent keys can GET `/orgs`, `/orgs/{key}`, `/orgs/{key}/authorization`, `/orgs/{org_id}/members`, `/orgs/{org_id}/members/{member_id}/authorization`, and `/orgs/{org_id}/role-scopes` under `/api/v1`, with membership/admin ACLs and no extra scope. Org-owned keys list their own org with Direct read access projected as `your_role: "admin"`; all writes, invites (including GET), and primary-org changes remain human-only for API keys. Scheduled-invocation keys remain denied; delegated `account:read` parity is unchanged.
- `ApiKey.allow_auto_connected_services` adds active, same-owner auto-provisioned and explicit platform-bound service IDs to restricted keys at auth-context construction; defaults to false. Human/delegated key listing, Agent Key login delivery (login options), and device-code approval/onboarding for the acting person's own account invoke shared provisioning. Public platform services create personal auto rows only; restricted platform services may create org rows only for allowlisted orgs reached through active Member/Admin memberships with `can_proxy()` and explicit grants. The 0.19.0 guarantee remains: org-targeted device approval/onboarding resolves existing org services and never provisions rows for the target org as a side effect of targeting. Grant removal or a switch to public reconciles those org auto rows away; startup also removes legacy public org auto rows. Legacy no-auth provisioning remains personal-only during membership traversal. JWT/API-key authentication itself never provisions rows.
...
- `nyxid-assistant` keys belong to one chat, cannot be channel route agents, and cannot be widened by native chat tools. Internal `ASSISTANT_ACCOUNT_SCOPE` (`assistant:account`) is granted only by a human chat acknowledgement or Full access mode; it is excluded from public scope assignment and grants no REST management authority.
...
- API: catalog GETs accept general API keys without proxy scope; service accounts remain rejected because catalog detail requires a User actor for restricted platform-grant membership lookup; delegated `account:read` parity is unchanged. Template metadata/live grants never include instance overrides; instance-backed private access and mounted-spec fallback use effective API-key inventory scope. `GET /api/v1/catalog?include_all=true` includes system services without auth (default filters to connectable); `GET /api/v1/catalog/{slug}/endpoints` fetches + parses the OpenAPI spec via hardened `api_docs_service::fetch_spec_json` (DNS pinning, 5MB limit, 60s cache); admin `POST/PUT /services` accepts all metadata fields with URL validation and length limits
...
### 10a. Catalog Skill Curation

- Dedicated Curation service accounts carry one live embedded grant with exact `DownstreamService` IDs, persisted write budget, optional Ornn proxy target, and sticky platform protection. Curation Ornn targets require an explicit existing `proxy_operation_policy` at grant issuance and live resolution; use a dedicated endpoint with exact skill CRU/read rules, since Ornn also has auth-only execution routes. Only platform admins issue/revoke grants or manage protected credentials/roles; ordinary SA request DTOs must reject grant/purpose/protection fields.
- The verified SA extractor confines Curation to grant-scoped `/catalog-curation` and ordinary HTTP `/proxy/{exact_ornn_catalog_id}`. No slug routing, `_nyxid_via`, WS, MCP, LLM, owner connection/node/endpoint inheritance, or self-management. General SA routing stays unchanged. Validate every SA JWT against its persisted token identity/scope/expiry/revocation and current credential generation, including MCP/introspection; generation changes atomically with secret rotation.
- Human and machine recommendation writes share `catalog_skill_service`: service metadata/skills, revision/history, durable actor/request receipt, and machine budget/fences commit in one MongoDB transaction. Pure no-op has no mutation/receipt/effects; metadata-only mixed changes need a receipt without phantom skill history. Keep external identity/billing/OIDC/discovery effects after commit and repair retries from current committed state.
- Preserve the existing names-based catalog digest; refs use a separate versioned skills digest. An instance name override suppresses catalog refs. NyxID owns recommendations, Ornn owns package bytes. Upgrade all auth/proxy/MCP replicas before enabling Curation credentials; disable/revoke before rollback. See `docs/SERVICE_ACCOUNTS.md#catalog-skill-curation`.

### 11. Oracle Relay (browser LLM pools)

Generic async task relay: any NyxID user or agent calls a browser-driven LLM through `/api/v1/oracle`. A **pool** owns the shared worker token and quotas. A **worker** is the supervised CDP daemon or the unchanged userscript. Browser details stay in worker clients. Full design: `docs/ORACLE_RELAY.md`.

- **Models**: all documents use UUID-string `_id` values and `COLLECTION_NAME`. `OraclePool` uses the polymorphic `user_id` owner. `OracleTask` stores prompt and response bodies, task status, lease, serde-defaulted `retry_count`/`max_retries`/`attempt_count`, optional `dispatch_attempt_id`, and terminal TTL. `OracleSession` stores multi-turn `conv_...` metadata. `OracleWorker` uses `{pool_id}:{label}` and has defaulted presence, capability, health, installation, and desired-state fields. `OracleWorkerCommand` stores leased, bounded command delivery. `OracleLoginSnapshot` stores only an outer-encrypted copy of an already end-to-end-sealed envelope with a one-hour TTL.
- **Queue and recovery** (`oracle_task_service`): MongoDB FIFO claim sorted by `created_at`; no in-memory state. `task_timeout_secs` defaults to four hours and `ack` refreshes the lease. Lease expiry and worker-reported pre-send `browser_recovery_exhausted` consume the same infrastructure retry budget and requeue to the FIFO front while retries remain. Exhaustion fails with `infrastructure_retry_exhausted`. Fresh claims increment attempts; idempotent reclaims do not. Capable clients echo `dispatch_attempt_id` on ack/result/pin/transcript so stale attempts cannot settle replacement work. `client_ref` remains submitter-scoped submit idempotency.
- **Worker identity and commands**: `POST /pools/{pool}/workers/allocate` reserves a unique label: server-generated by default, or an operator-chosen `label` (validated; an existing unbound legacy row is adopted, a label bound to another installation is refused with 11014). New CDP workers persist a stable installation ID, so two installations cannot share one label. Managers can queue `drain`, `resume`, `restart`, `relaunch_browser`, `relogin`, and `upgrade`; `session_import` is internal. `DELETE /pools/{pool}/workers/{label}/commands/{id}` (`worker cancel-command`) withdraws a queued or delivered-but-unexecuted command; delivered ones are dropped by the worker on its next heartbeat via `cancelled_command_ids`. `DELETE /pools/{pool}/workers/{label}` (`worker forget`) removes a stale presence row and its commands and releases the label's session affinity; it refuses online or busy workers unless `force=true`. Commands arrive only through capability-gated heartbeats, use delivery leases and bounded redelivery, and carry metadata-only audit records. Legacy userscripts advertise no capabilities and never receive commands or new protocol shapes.
- **CDP resilience**: transient HTTP failures retry forever with capped jittered exponential backoff. CDP disconnects reconnect, repeated CDP and task-level failures relaunch configured Chrome, and tab failures reopen ChatGPT and restore the project URL. Task recovery has a bounded failure count. The mode `0600` state file stores task ID, attempt ID, conversation URL, phase, transcript baseline, and failure counters only. Persist `send_attempted` before Send. After recovery, extract a completed matching turn, wait for an existing pending turn, or send only from a known pre-send phase. An uncertain post-send transcript must fail with `prompt_delivery_uncertain`; NEVER resend that prompt. Reasoning-level selection (`selectModel`) runs under a cooperative deadline with an abort signal, opens the structural composer pill regardless of locale, only treats menus that appeared after the pill click as the picker, never clicks an unrecognized entry, and is followed by a bounded composer-unobstructed guard before typing and before Send; the model family/tier is verified before the effort pill and again before Send. `require_model_match` defaults true on pools/tasks; switcher mismatch or recognized effort evidence that cannot verify fails pre-send as `model_unavailable` (unrecognized Tools/attach controls do not). Families parse generic major/minor versions; compare minors only when both exist. Exact tier-only entries require matching family context; at most one exact-family submenu is opened with a new menu identity baseline. Plain `-pro` and `extended`/`扩展` prefer Pro Extended; `standard`/`标准` explicitly chooses Standard. Acks run `page_ready` -> `selecting_model` (second ack carries metadata-only `phase_detail`) -> `ready_to_send` -> `sent`. Raw picker labels are logged only with `NYXID_ORACLE_LOG_PICKER_LABELS=1`. Model family/tier verification (`selectModelSwitcher`) prefers the header switcher and otherwise accepts a unique compact `<version>\n<tier>` menu button inside the composer form (also recognized as picker items); the marked switcher trigger is never an effort-pill candidate.
- **Failure attribution and capacity**: bounded `failure_detail` is `stable_code@phase`, never error text; preserve the last underlying cause through infrastructure exhaustion and delivery uncertainty. One metadata-only structural probe accompanies settlement. `page_crashed` replaces the tab. After Send, empty output and scoped ChatGPT error UI reload once, recheck the send fence, then wait even without a generation indicator or fail as task errors; persistent pre-send error banners use one reload and one DOM-shape relaunch cycle. `usage_limit_reached`/`model_unavailable` use `excluded_worker_ids` and at most 20 capacity reroutes, independent of infrastructure retries; capacity failures settle with the original code when no eligible alternate has recent `last_seen_at` from heartbeat/ack/claim (busy workers count), or the task is account-pinned. Usage cooldown defaults to 900 seconds (`NYXID_ORACLE_USAGE_COOLDOWN_SECS`); heartbeats continue without claims. Pool status exposes embedded bundle version and outdated online count; results and capacity reroutes retain canonical switcher/effort observations, with a diagnostic `selecting_model` ack before strict failure. Presence ignores and unsets legacy `page_url`. Old workers remain additive-compatible.
- **Pool-wide login**: `nyxid oracle login <pool>` performs password, OTP, SSO, and Cloudflare work in local Chrome, captures allowlisted ChatGPT/OpenAI cookies and storage through CDP, and encrypts locally with AES-256-GCM using HKDF-SHA256 from the raw pool worker token. The manager uploads only a 512 KiB-capped sealed envelope and token hash verifier, then stops Chrome and deletes the temporary capture profile. The server validates the verifier, wraps the sealed envelope with `EncryptionKeys`, TTLs it for one hour, and queues `session_import` to capable workers. Workers defer import until task settlement unless a logged-out task cannot continue, decrypt locally, import through CDP, verify the logged-in DOM, and report per-worker results. A device-bound rejection is a clear verification failure. The persisted server state cannot derive the HKDF key, but the live backend sees bearer tokens while authenticating worker requests.
- **Worker token and bundle**: `nyx_owk_...` is 32 random bytes; only SHA-256 is stored, and create/rotate shows it once. Install and login use a local token file, environment variable, or hidden prompt and NEVER rotate implicitly. The backend embeds `worker.mjs`; manager and worker-token endpoints return source, version, SHA-256, and the exact `playwright-core` version. CLI install and worker upgrade verify SHA-256. Worker upgrade runs npm only when the installed `playwright-core` differs from the manifest pin (bundle-only upgrades need no npm or network), using the install-time absolute npm path (`NYXID_NPM_EXECUTABLE`) with node's directory on the daemon `PATH`, and restores `package.json` on failure. launchd or systemd restarts the worker after upgrade.
- **Org worker enrollment**: `worker install --pool <pool>` automatically enrolls active Member/Admin contributors in org-visible pools using NyxID authentication; owners/admins can also enroll in pools they manage. No shared token is requested. CLI generates and privately persists a `nyx_owi_` installation credential before `POST /pools/{pool}/workers/enroll`; the server stores only its hash. Every worker route checks actor/org activity, membership ID + join timestamp, pool token epoch, label, and installation. Members manage their own contributed workers; admins manage all. Enrollment inventory is capped at 256 separately from `max_workers` dispatch concurrency. Rejoining after revocation/rotation requires authenticated `install --force` and a fresh credential (11016). New installs use their own browser accounts; shared-login capabilities and envelope access are excluded. Existing/explicit pool-token installs retain saved-login support.
- **ACL and compatibility**: `ensure_can_submit` uses visibility and `org_service::resolve_owner_access`; `ensure_can_manage` gates pool mutation, legacy worker allocation, and login fanout. Worker management also permits eligible contributing owners through `oracle_worker_enrollment_service`. Worker commands bind an immutable generation to prevent label-reuse races; enqueue/cancel/forget fence the authorized worker in Mongo transactions. Bundle download requires ordinary authentication; installation proves enrollment or manager access and verifies the worker endpoint's pool ID. Unauthorized task/session reads are not-found-shaped. Startup rewrites exactly the retired pool default `chatgpt-5.5-pro` to `chatgpt-6-pro` idempotently. All new fields are optional or serde-defaulted. Existing rows, deployed userscripts, and old CDP workers remain valid.
- **Privacy**: prompt, response, transcript, attachment, attachment filename, generated files, generated filenames, session, cookie, storage, raw-token, conversation-URL, and signed-image material must NEVER appear in logs or audit. Prompt, response, and generated artifact bodies live only on the task document and expire after `ORACLE_TASK_RETENTION_DAYS`. Worker state contains metadata only. Login snapshot logs and audit contain IDs, sizes, target counts, status, and stable error codes only. Secret-bearing structs need redacted custom `Debug`.
- **CLI**: consumer and pool commands remain. Worker operations add `worker list/show/install/start/stop/status/logs/uninstall`, control verbs, and `worker upgrade`. `worker list` reports only online workers by default and `--all` adds offline rows with their last-seen age. `worker upgrade --pool <pool>` waits for and verifies the installed local worker. Adding `--label` queues an asynchronous remote upgrade. `oracle login <pool>` performs local capture and pool fanout. Installs live under `~/.nyxid-oracle/<pool>/`, with named profiles under `profiles/<name>/`, and reuse the node daemon's launchd or systemd pattern.

Key files: `models/oracle_{pool,task,session,worker,worker_command,login_snapshot}.rs`, `services/oracle_{pool,task,session,worker,login_snapshot,worker_bundle}_service.rs`, `handlers/oracle_{pools,tasks,worker,workers,worker_bundle}.rs`, `cli/src/commands/oracle.rs`, `cli/src/commands/oracle_worker_daemon.rs`, `integrations/oracle/cdp-worker/worker.mjs`, `integrations/oracle/nyxid_oracle.user.js`.

### 12. Auth Device-Code Login

First-party RFC 8628-style login: plain `nyxid login` and `--device` display a short `user_code`; a human chooses account access or a restricted Agent Key in the web UI or mobile app. Explicit `--agent-key` remains restricted-only.

- **Storage and rollout**: legacy `auth_device_codes` remains account-only. New `/auth/device/v2/{request,poll,poll-web}` uses isolated `auth_device_codes_v2`, `nyx_adc2_` poll secrets and `2-XXXX-XXXX` human codes. Shared preview/decisions choose storage by code version. Never retry v2 through legacy polling. Both collections store HMACs of codes, sanitized context and encrypted deliveries; raw codes and token plaintext never persist. Terminal TTL uses `purge_at` only after cleanup; an indexed expiry sweep retains unfinished revocations. Old replicas and legacy TTL indexes cannot consume/delete v2 grants.
...
- **Grant decisions and delivery**: only first-party human sessions/access JWTs without an OAuth client ID can decide grants. Restricted selection uses `/auth/device/approve-agent-key`, so an old replica cannot silently issue account access. Issuance and approval commit together. Decrypt before atomically claiming `approved` -> `delivered`, retaining retryability on KMS failures. Account browser delivery atomically revokes approval bearer credentials and creates the cookie session. Restricted browser delivery returns a one-time CLI handoff without cookies; handoff redemption and direct polling have one winner. Abandonment revokes only its session/child, preserving reused parent configuration and other consumers.
- **Rate limiters**: `auth_device_{request,poll,approve,approve_per_user,preview}_limiter` (`/poll-web` shares `poll`)
- **CLI output**: prints `user_code` plus the bare `verification_uri` -- never `verification_uri_complete`; manual entry preserves the deliberate code-transfer anti-phishing step. `-c` / `--clipboard` best-effort copies only the user code before browser opening; failure prints one stderr line and continues. Clipboard conflicts with `--password` and `--code`, but is allowed with `--callback` for the device-code fallback. JSON/no-wait modes print the code without copying or opening a browser. Explicit device/Agent Key login on stdin+stderr TTYs prompts `Open in your browser? [Y/n]` and opens via `crate::browser::open_browser`; failure leaves manual URL entry available. Without a TTY, explicit modes skip the prompt and poll.
- **CLI dispatch**: plain blocking login deliberately defaults to selectable device v2, including desktop: it is the only terminal-originated flow with requester attribution (IP, timezone, origin, screen) and the human's account/restricted Agent Key choice (#1535). This motivated 0.16.0 (`78cb26b`), whose commit message omitted the rationale. `--callback` selects the legacy local browser callback (full account session, no code entry or requester review), with CannotOpenBrowser -> device-code fallback. `NYXID_LOGIN_NO_DEVICE_FALLBACK=1` is its legacy alias through the same dispatch path. `--callback` conflicts with password/device/Agent Key/code/no-wait flags and rejects `--output json` or `login resume` at the top of CLI dispatch (no callback JSON/resume contract). Without `--callback`, explicit device/Agent Key/code/resume/JSON/no-wait modes keep the exchange. `--no-wait --output json` never waits or opens a browser; `login resume <REQUEST_ID> [--once]` uses a mode-0600 local handle bound to profile/destination, persists backoff/deadlines and returns distinct stable JSON errors. Poll secrets never appear in output.
- **Verification page** (`/login/device`): public preview with explicit computer/phone choice. Optional `?user_code=` prefills once on mount after `userCodeSchema` validation, formatted through shared `formatAuthDeviceUserCodeInput`; replace navigation immediately removes the param from the URL. The sign-in `return_to` carries the normalized code back to this same-origin approval page, preserving its query through password/MFA login without expanding return-URL trust. Malformed input leaves the field empty with an error message. No request or decision occurs on mount, focus, typing or reconnect; Continue explicitly previews. Review echoes the formatted code through both final confirmation paths and asks the approver to match it against the requesting device/terminal and reject a mismatch (RFC 8628 §§3.3.1, 5.4). Account approval and key permission confirmation require first-party human authentication and explicit decisions with >=750 ms throttling. Legacy codes only offer account access. `/login/code` is a mint page without a code input and remains unaffected. Phone preview, including polling for the requesting computer, leaves browser account cookies/storage untouched. Login-grant routes suppress telemetry even after SPA navigation from an opted-in page.
...
- **One-time codes**: web/mobile settings mint through `/auth/login-code`; public `/redeem` atomically claims and issues either grant. Issuer-only status, cancellation and revocation expose no secrets. Five-minute codes are limited by source and owner and stored only as HMACs. Browser handoff extends the initial 60-second delivery window to approval + 300 seconds; repeated handoffs never extend that fixed deadline. First-party JWT `sid` checks live session state during protected access and refresh; issuer revocation cascades to MCP and invalidates refreshed access too.
- **Local identity lifecycle**: login replacement, refresh snapshots and logout use a profile generation under the refresh lock. An in-flight request cannot retry another login's token, including a replacement at the same destination. Bounded logout clears only the matching generation. Explicit credentials do not require profile storage or fall back to another identity.
...
- **Anti-phishing and privacy**: the CLI prints the bare verification URL. The public `/login/agent-key` page accepts `?user_code=` under Rule 12's contract: validate with `userCodeSchema`, prefill once through shared `formatAuthDeviceUserCodeInput`, immediately remove the param with replace navigation; malformed -> empty field plus message. No request/decision on mount, focus or typing; preview requires explicit Continue. Phone QR contains a user code, never a credential; scanning/deep linking prefills both web and mobile. Review echoes the code through final confirmation and instructs the approver to match the requesting device/terminal and reject a mismatch (RFC 8628 §§3.3.1, 5.4). Approval always requires review, key selection, permission confirmation, and an explicit decision with >=750 ms throttling. `/login/code` minting is unaffected. The computer's QR path uses anonymous preview polling at >=5 seconds, component state only, and clears temporary state at terminal/unmount without storing account credentials or cookies.
- **CLI profiles**: atomic mode-0600 `token`, `auth_kind=agent_key`, safe `agent_key.json`, and existing `base_url` handling. Stale account access/refresh/user files are removed. Saved Agent Key credentials never refresh, parse JWT expiry, or prompt for account login. `whoami`/`status` use live self metadata; explicit/env credential precedence stays unchanged. `session refresh` exits 3; bounded logout clears the matching local login after best-effort server revocation, preserving a newer concurrent login.
...
- `/auth/device/v2/{request,poll,poll-web}` and shared `/auth/device/{preview,options,approve,approve-agent-key,deny}` -- selectable login; legacy request/poll/poll-web remain account-only (Rule 12)
...
- `/assistant/nyxagent/*` -- human-only, default-on NyxAgent conversations/history/rename/delete, turn SSE, Stop, acknowledgement decisions (`POST /conversations/{id}/acknowledgements/{ack_id}`), owner-only tool images (`GET /conversations/{id}/attachments/{attachment_id}`), mode switches (`PATCH /conversations/{id}/access-mode`), and cached profile discovery; all routes enforce the engine flag and acting-person ownership
...
- `/oracle` -- browser LLM relay (Critical Rule 11). Consumer routes cover pools, tasks, attach, status, and sessions. Manager routes add worker allocation/list/show/commands, login-snapshot fanout, and the embedded worker bundle. `/oracle/worker/{task,heartbeat,ack,result,pin-conv-url,transcript,bundle,login-snapshots/*}` authenticates with the pool worker token inside each handler and outside JWT middleware. The Oracle route groups have a 16 MiB body cap and normal global rate limiting. The manager login-snapshot route has a smaller request cap based on its 512 KiB decoded envelope limit.
- `/channel-platforms` -- authoritative adapter metadata, registration fields, managed onboarding, non-secret credential-configured flags and media capabilities; JWT/session, agent key, service account and delegated `account:read` accepted.
- `/channel-relay/messages/{message_id}/attachments/{index}` -- bounded inbound media download using the assigned live agent key or exact-message reply token without consuming its JTI. Explicitly denied to delegated/service-account/relay tokens. Stores inbound routing metadata only; never media bytes, outbound filenames or captions (ADR-013).
- `/channel-bots` -- channel bot registration CRUD + PATCH for platform verification material
- `/admin/platform-credentials[/{provider}]` -- admin-only descriptor listing (GET), field updates (PUT/PATCH, null clears), DELETE; generated Verify Token is admin-readable, app secrets never returned
- `/channel-bots/managed-onboarding/{platform}` (GET), `/channel-bots/managed-onboarding/{platform}/complete` (POST), `/channel-bots/{id}/reregister` and `/channel-bots/{id}/managed-setup/repair` (POST) -- human-only router; writes share a per-user rate limit
- `/webhooks/channel/{platform}/platform` -- adapter-opted-in public GET handshake / POST signature-verified dispatcher
...
- `/channel-relay/send` -- target-addressed proactive send, default-off human opt-in (`allow_agent_initiated`); assigned live API key or human owner session (including first-party CLI access JWT), shared per-conversation rate limit before auth, payload-bound 24h idempotency claims, metadata-only storage. Adapters declare outbound capabilities and classify permanent target refusals; unsupported adapters explicitly refuse dispatch.
- `/channel-relay/conversations` -- API-key-only discovery of active assigned conversations, addressability, opt-in, capabilities, and last-message timestamp; paginated, metadata only.
- `/channel-relay/reply/update` -- edit a previously-sent reply by the upstream `platform_message_id`; same dual auth. A reply token may edit only if its JTI already exists in `reply_token_uses` (proves it was used to send first). Native edits: Telegram (including telegram-new), Discord, Slack, Lark, and Feishu. WhatsApp, X, and OpenClaw return `edit_unsupported`. API keys can also edit agent-initiated messages; reply tokens remain bound to anchored replies.
...
- `/webhooks/channel/{telegram,discord,lark,feishu,slack,whatsapp}/{bot_id}` -- adapter-driven public POST receivers; WhatsApp also uses GET for Meta subscription verification
...
New browser chats default to `assistant:nyxagent-engine` (on), then the experimental Direct flag, then Aevatar. NyxID owns `nyxa-*` transcripts and detached turns; the `llm-nyx` catalog row owns the target and NyxAgent owns disposable live context. A stable per-conversation assistant key authenticates upstream; failed/Stopped turns clear the context binding, and later turns send a bounded recap. Active fences expire after 2100 seconds and the next send transactionally records `turn_lost` before reclaiming; settlement retries stop after five minutes. The browser polls selected history while active or awaiting acknowledgement and renders one inline note at the latest context reset. Each chat owns a restricted key, encrypted credential row, and `assistant_acknowledgements` transcript records. Native `nyxid__*` account tools dispatch directly through services with owner checks, metadata audit and non-blocking human acknowledgement; refusals are MCP `isError` results. `access_mode` defaults to `ask`; human-confirmed `full` atomically enables all connected services/nodes and account/destructive tools without cards, while preserving secret exclusions and the ban on modifying assistant keys. Switching back retains individually approved service IDs and removes broad service/account authority. Conversation keys always allow nodes because service consent covers its node route. Platform-source services without a UserService get the same Ask-mode card; Allow stores the catalog ID on the chat key's `allowed_platform_service_ids` (chat keys only), and `nyx__connect_service` mints hosted links for chat keys without an allowlist entry. Execution/mutation requests add assistant MCP audits; read-only discovery does not. Mode switches reject live turns and audit `assistant_access_mode_changed`. An allowed card resumes the assistant with a visible continuation turn, queued until a running turn settles (not after Stop); each turn's instructions also list card decisions made since the previous user message, identifiers only. Chat-key `tools/call` requests during a live turn are recorded as metadata-only `active_turn.activities` (bounded) and copied onto the settled reply so the browser can show what the assistant is doing. Verified PNG/JPEG/GIF/WebP tool responses (2xx, <=5 MiB, type and magic bytes agree) become a leading text note plus, for non-chat callers, MCP image content (<=1 MiB inline, NyxAgent caps responses at 2 MiB); chat keys get the note only because NyxAgent stringifies and truncates results at 10,000 characters and cannot pass pixels; for chat keys with a live turn they are also envelope-encrypted into `assistant_attachments` (<=8 per turn, deleted with the conversation and in user purge), listed as `attachments` on the turn and reply, and served owner-only at `GET /assistant/nyxagent/conversations/{id}/attachments/{attachment_id}` with nosniff and a sandbox CSP. `ToolResponse.text` stays byte-identical for existing consumers and receipt digests. The per-user browser draft preference never changes an existing chat implicitly. Existing actor history and action routes remain available. No new NyxID environment variables; the NyxAgent deployment must set `NYXAGENT_SERVICE_SLUG=llm-nyx` for recursion protection. See `docs/chat/08-nyxagent-engine.md` for the normative contract and deployment prerequisites.
...
WhatsApp uses the **Business Platform / Meta Cloud API**, not the consumer WhatsApp Business App or Twilio. **Access Token** authenticates Graph calls; **Phone Number ID** is `platform_bot_id`; **Meta App Secret** verifies raw POST HMACs; optional **WABA ID** uses the generalized `app_id` storage column but is exposed as `waba_id`. The generated one-time **Verify Token** configures the Meta dashboard GET handshake; token/secret rotation and Verify Bot must preserve its hash. Subscribe the callback to `messages` and manually subscribe the app to the WABA. Filter app-wide events by Phone Number ID. Registration fields, verification secrets, acknowledgment policy, reply context, and retry deduplication belong to adapter hooks; `services/channel_adapters/mod.rs::resolve_adapter` is the single registry. WhatsApp alone opts into atomic inbound admission: an existing coordination-store claim is committed with metadata in a MongoDB transaction, scoped to bot/owner/wamid, with an in-transaction legacy-row check and expiry/replacement fencing. X retains lookup dedup and Telegram edits retain their behavior. Provider ACK remains immediate; callback failure, timeout or crash after admission is not automatically retried. Claims are released only before admission; committed markers last 30 days.

WhatsApp send outcome hooks preserve every accepted component ID (split text, media, audio captions; max 64), final-ID API compatibility, expected count and certainty. Default hooks preserve other adapters’ `send_bound_reply` overrides. A confirmed Graph refusal before any accepted component releases an initiated claim; ambiguous or partial sends retain it for the existing finite 24h retention and persist known IDs without marking the send complete. Signed, phone/WABA-filtered individual receipts store bounded IDs, per-status provider times and numeric codes only, with UUID v4 `_id`, unique identity key and 30d TTL. Group/participant receipts are rejected. Authorized message pages batch-join exact owner/bot/phone/WABA/recipient/wamid receipts; read implies delivered. Whole-reply status requires all components. Partial/unknown and legacy final-only rows cannot claim whole-reply delivery; group routes never imply all-participant delivery. No raw webhook, error prose, content queue or unmatched receipt API. Conversation IDs are `wa_id` digits. See `docs/CHANNEL_BOT_RELAY.md` for setup and delivery limits.
...
CHANNEL_RELAY_INITIATE_RATE_LIMIT_PER_SECOND=1 # Per-conversation unsolicited message limit
...
nyxid login --device                    # Explicit selectable device-code login (also the plain-login default)
nyxid login --callback                  # Full account session via local callback; legacy env alias: NYXID_LOGIN_NO_DEVICE_FALLBACK=1
nyxid login -c                          # --clipboard copies only the user code; JSON/no-wait modes skip clipboard/browser
...
# Oracle relay
nyxid oracle ask <pool> <prompt> | result <task_id> | status <pool>
nyxid oracle pool create|list|show|update|rotate-token
nyxid oracle worker install --pool <pool> [--label <label>] [--worker-token-file <path>] [--profile <name>]
nyxid oracle worker list <pool> [--all] | show <pool> <label>   # online only unless --all
nyxid oracle worker start|stop|status|logs|uninstall --pool <pool> [--profile <name>]
nyxid oracle worker drain|resume|restart|relaunch-browser|relogin <pool> <label>
nyxid oracle worker forget <pool> <label> [--force]   # remove a stale worker row + its commands; releases its session affinity
nyxid oracle worker cancel-command <pool> <label> <command-id>   # withdraw a queued/delivered-but-unexecuted command (e.g. a bad upgrade)
