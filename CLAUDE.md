...
- **Authorize and consent**: only enabled Gate clients enter the gate. Local evidence must be no older than 60 seconds, with a 60-second maximum spread. `prompt=none` creates no session; `nyx_connect=force` opens the checklist. Advise leaves authorize unchanged. Validated parameters, including consumed PAR parameters, stay server-side. Ready freezes result selections, transitions to `ReadyForConsent`, and mints a fresh 15-minute consent JWT binding result/session/subject/client/parameters. Required rows cannot be deselected; URL and form hints are not authority. Decision rejects omissions and rechecks activation, rollout, and local authority. Code publication, consent, and completion commit together with a revision fence. Terminal redirects use only the stored validated URI and original state; the expiry sweep never delivers browser callbacks.
