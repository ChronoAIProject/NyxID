...
- `ServiceBilling.byok_pricing` / `platform_key_pricing` are optional credential-class lanes: the existing fields are the primary price; optional `components` add unique metrics. Either lane present supersedes legacy platform billing; missing lane and `NoAuth` are free. Synced components charge independently; any pending/failed component adds at most one legacy fallback charge (or free); resale stays independent. Primary Lago codes remain `platform_svc_{slug}_{byok|pk}`, components append `_{metric}`, with durable component cleanup/reconcile. Omitted lanes/components preserve them; null/[] clears components. Token/image capture checks every configured component independent of slug/sync; priced token classes subtract caches included in OpenAI/Gemini input, but keep Anthropic's separate cache counts; provider `TokenBreakdown` remains unchanged. Allowances match identical configured metrics; default BYOK primary, then platform primary, then legacy. Prices support `PRICE_FRACTIONAL_DIGITS = 12`, optional pico rates plus truncated legacy micros; multiply exactly in i128, truncate gross costs to micros, ceil exact wallet remainders to whole credits. Keep funding precedence, plan charge IDs and ledger canonical encoding unchanged. Upgrade ALL replicas before authoring component prices, new-metric allowances, or prices beyond six decimals; old replicas cannot deserialize the new enum units. See `docs/PLATFORM_KEYS_AND_INFERENCE.md`.
