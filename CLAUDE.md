...
- **CLI dispatch**: plain blocking login uses selectable device v2 and offers browser approval. `NYXID_LOGIN_NO_DEVICE_FALLBACK=1` preserves the legacy browser-callback strategy and its established browser-open fallback. Explicit device/Agent Key/no-wait modes use the exchange. `--no-wait --output json` never waits or opens a browser; `login resume <REQUEST_ID> [--once]` uses a mode-0600 local handle bound to profile/destination, persists backoff/deadlines and returns distinct stable JSON errors. Poll secrets never appear in output.
- **Verification page** (`/login/device`): public preview with explicit computer/phone choice. Code input starts empty; query parameters are stripped and ignored. Account approval and key permission confirmation require first-party human authentication. No decision occurs on typing, mount or reconnect. Legacy codes only offer account access. Phone preview, including polling for the requesting computer, leaves browser account cookies/storage untouched. Login-grant routes suppress telemetry even after SPA navigation from an opted-in page.
...
nyxid login --device                    # Headless browser-assisted login;
                                        # NYXID_LOGIN_NO_DEVICE_FALLBACK=1 selects legacy browser callback for plain blocking login
