...
- `/admin/audit-log` -- exact counts and unchanged substring filters; count and page queries run concurrently. Unfiltered counts use the existing `_id` index; global-search-only counts use one collection pass instead of seven full index scans, and their pages use the existing sort index. Narrowing predicates retain normal index selection. Event-type filter options are derived display data cached per process/AppState for 30 seconds; audit rows and counts are never cached.
