...
- **Worker identity and commands**: `POST /pools/{pool}/workers/allocate` reserves a unique label: server-generated by default, or an operator-chosen `label` (validated; an existing unbound legacy row is adopted, a label bound to another installation is refused with 11014). New CDP workers persist a stable installation ID, so two installations cannot share one label. Managers can queue `drain`, `resume`, `restart`, `relaunch_browser`, `relogin`, and `upgrade`; `session_import` is internal. Commands arrive only through capability-gated heartbeats, use delivery leases and bounded redelivery, and carry metadata-only audit records. Legacy userscripts advertise no capabilities and never receive commands or new protocol shapes.
