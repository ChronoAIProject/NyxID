...
- **Privacy**: prompt, response, transcript, attachment, session, cookie, storage, raw-token, conversation-URL, and signed-image material must NEVER appear in logs or audit. Prompt and response bodies live only on the task document and expire after `ORACLE_TASK_RETENTION_DAYS`. Worker state contains metadata only. Login snapshot logs and audit contain IDs, sizes, target counts, status, and stable error codes only. Secret-bearing structs need redacted custom `Debug`.
- **CLI**: consumer and pool commands remain. Worker operations add `worker list/show/install/start/stop/status/logs/uninstall`, control verbs, and `worker upgrade`. `oracle login <pool>` performs local capture and pool fanout. Installs live under `~/.nyxid-oracle/<pool>/`, with named profiles under `profiles/<name>/`, and reuse the node daemon's launchd/systemd pattern.
