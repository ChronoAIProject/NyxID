...
- 12100-12110 app requirements and connection validation: 12100 `AppRequirementsInvalid`, 12101 `AppConnectLinkNotFound`, 12102 `AppConnectLinkExpired`, 12103 `AppConnectLinkCompleted`, 12104 `AppConnectLinkCancelled`, 12105 `RequirementNotSatisfiable`, 12106 `RequirementNotMet`, 12107 `ServiceValidationRejected` (422), 12108 `ServiceValidationUnavailable` (503), 12109 `ServiceValidationRateLimited` (429), 12110 `AppConnectResultMismatch`
...
- Catalog `platform_key` uses the existing encrypted master credential and live public/restricted polymorphic-owner grants. Every platform-bound resolver must call the shared ACL before decryption; no anonymous grant, no node routing or user-controlled destination. Gateway-URL providers cannot enable platform keys. Restricted ACLs batch memberships/active owners; catalog, keys (including provisioning/reconciliation), MCP and LLM listings share one request membership snapshot across service/owner checks. Reuse loaded providers or batch them once; `available_with_grants` must perform no database calls. Never cache these grants across requests. Legacy absent configs preserve public internal master-key behavior. `UserService.credential_binding` is optional; `POST/PUT /keys` and hosted/MCP flows use `use_platform_key`. Switching to platform retains the user's key. Automatic grants reconcile personal/org rows; scoped agent auto-connected grants also include explicit platform bindings.
- `ServiceBilling.byok_pricing` / `platform_key_pricing` are optional credential-class lanes. Either present supersedes legacy platform billing: matching synced lane charges, missing lane is free, pending/failed matching lane falls back to legacy. `NoAuth` is never lane-charged. No lanes preserves legacy behavior; resale stays independent. Stable Lago codes are `platform_svc_{slug}_byok` / `platform_svc_{slug}_pk`, with independent durable cleanup and reconcile retries. Keep full plan charge IDs, funding precedence, usage identity and ledger canonical encoding unchanged. Old admin payloads omitting lanes preserve them; null clears. Token capture checks both lanes regardless of slug; MCP estimates only token-metered services. Allowances match the charged request metric, with optional admin `metric` for mixed lanes; omitted metric defaults to BYOK, then platform key, then legacy.
...
- `ApiKey.allow_auto_connected_services` adds active, same-owner auto-provisioned and explicit platform-bound service IDs to restricted keys at auth-context construction; defaults to false. Key listing, Agent Key login delivery (login options), and device-code approval/onboarding for the acting person's own account invoke shared provisioning, which may idempotently create org-owned auto-connected rows only through that person's own active Member/Admin memberships with `can_proxy()` and explicit platform-key grants. The 0.19.0 guarantee remains: org-targeted device approval/onboarding resolves existing org services and never provisions rows for the target org as a side effect of targeting. Grant removal reconciles those rows away. Legacy no-auth provisioning remains personal-only during membership traversal. JWT/API-key authentication itself never provisions rows.
...
App requirements, hosted repair, and gated OAuth share one session engine. The as-built design is in `docs/superpowers/plans/2026-09-14-app-required-services-onboarding.md`; API contracts are in `docs/API.md`.

- **Rollout**: `APP_CONNECT_ROLLOUT` defaults to `disabled`. `allowlist` requires an active client, platform-admin-granted `app_connect_capability_enabled`, and an active org owner in deployment-configured `APP_CONNECT_ALLOWED_ORG_IDS`. The MongoDB mode override refreshes on the broker policy interval; stale revisions never replace newer snapshots. Capability is never self-grantable or inferred from DCR scopes. Disabled rollout makes App Connect routes not-found-shaped and leaves ordinary authorize unchanged. `public` requires separate review and is not offered in the admin UI or CLI.
- **Validation**: `service_validation_records` stores attempt-fenced observations bound to profile version, execution-authority digest, and credential material revision. Read access shows the key; a check requires the actual caller's proxy permission and, for access tokens, the REST proxy execution scope. Probes reuse coordinated OAuth refresh without updating last-used or writing credential status. Provider answers may be reused for five minutes; internal aborts and transport/node failures have zero reuse windows. Direct egress pins public DNS per attempt, disables redirects, and bounds decoded bodies within four seconds. Gzip decoding is explicit and isolated from shared proxy clients. Node egress uses the owner's network boundary and requires `no_redirect_proxy` capability; offline nodes never imply success. MongoDB leases key provider cooldown by stable credential ID plus profile, independent of aliases or metadata/digest edits. Active admission is two per human session (user ID fallback), sixteen per app, and thirty-two per deployment. Node observations bind the routed node/slug and the agent's local credential revision, advertised on status updates and stored with the fenced connection owner; missing revisions make observations non-reusable. Node probes send uncredentialed paths and let the agent inject once. Platform bindings use the live catalog credential and server-only transport even if a personal key or legacy node binding remains; platform credential rotation invalidates observations. Checks run only on explicit actions; keep the separate Agent Key scope-verification UI.
- **Manifests and evaluation**: `app_requirement_manifests` contains immutable UUID-string rows with per-client monotonic versions. Publication freezes catalog IDs, active seeded-prefix membership, and a profile/version for every alternative slug, and advances `OauthClient.current_manifest_version` in the same transaction. Both `Advise` and `Gate` require rollout/capability. Private no-credential requirements cannot depend on consent to the same app; they need an independent prerequisite. Local evaluation performs no provider I/O, decryption, or refresh. Org candidates require `CredentialSource::Org { allowed: true }`; explicit eligible selections win, followed by Met/Included, Unknown, NeedsReauth, Broken, evidence age, last use, and ID. Disabled services are never auto-enabled; eligible no-credential services use the existing auto-provision path and appear as Included. Shadowed selections are Unsatisfiable with `slug_shadowed` under proxy precedence.
- **Status and results**: `/api/v1/app-requirements/status` requires a developer-app user token, discloses only manifest services, and is limited to 30 requests per user per minute. `granted_to_caller` comes from that token's own allowlist. The GET may auto-provision eligible no-credential services. `app_requirement_results` binds the client, person, manifest version, and selections; identical selections reuse the newest unexpired row without extending its one-hour BSON `expires_at` TTL. Automatic selections are not explicit preferences. Results confer no execution grant.
- **Hosted links**: `app_connect_links` binds app/person/manifest and has App or Authorize origin. Page capabilities are hashed, redeemed once by the bound human Session, and cleared from hash/storage. Before login, tab-local storage preserves the fragment capability; it must never enter `return_to`. Subsequent reads require the same subject; app reads require both token client and subject. Org resource write access never substitutes for session ownership. Child ConnectLinks carry parent/requirement/reauthorization fields and the shared `scopes` field and server-derived internal return URLs. Child cancellation and parent attempt invalidation commit together; item and terminal writes compare durable attempts/revisions. Starting an item over cancels its previous pending child; cancelling or expiring a parent cancels all its pending children. TTL is 30 minutes plus one 15-minute extension per Met requirement, capped at two hours, with one-day terminal retention. Disabled choices retain Change/optional Skip and a management link; only Connect/Re-check are disabled, and window focus re-reads local readiness. Repair completion issues no tokens and changes no consent; its callback reports `grant_update_required` when selected services exceed stored consent.
- **Authorize and consent**: only enabled Gate clients enter the gate. Local evidence must be no older than 60 seconds, with a 60-second maximum spread. `prompt=none` creates no session; `nyx_connect=force` opens the checklist. API-mode access tokens can request a code or handoff only for their own client (first-party tokens without a client and Session callers retain their existing behavior); the gate applies the same client check. Successful and failed callbacks preserve registered query pairs and replace protocol response fields. Advise leaves gating disabled. Validated parameters, including consumed PAR parameters, stay server-side. Ready freezes result selections, transitions to `ReadyForConsent`, and mints a fresh 15-minute consent JWT binding result/session/subject/client/parameters. The subject-bound `/oauth/consent-presentation` verifies the consent JWT and supplies the registered client name, mandatory IDs, and selectable IDs. Required rows cannot be deselected; URL hints never make a row mandatory or identify the app. Bound decisions reject IDs outside that presentation selector. Decision rejects omissions and rechecks activation, rollout, and local authority. Stale evidence or changed authority CAS-reopens ReadyForConsent for repair, clears and expires the result atomically with rotating the consent nonce while preserving choices; old consent tokens cannot complete it. Code publication, consent, and completion commit together with a revision fence. Terminal redirects use only the stored validated URI and original state; the expiry sweep never delivers browser callbacks.
- **Token boundary**: stored service IDs are authoritative; resource URIs are derived display data. Code exchange, refresh, and token exchange may only narrow them. Active, accessible slug substitution fails closed; inactive/tombstoned IDs remain bound for Disable/Enable, while missing IDs and inaccessible active rows narrow the issued boundary. Catalog delegation applies the same normalization before catalog ceilings, keeping normalized IDs and URI metadata together. A tombstone never grants the replacement that reuses its slug. Neither readiness nor repair silently widens an existing grant.
- **Login context**: `oauth_authorize_contexts` stores validated snapshots with UUID-string IDs, BSON dates, and a five-minute `expires_at` TTL. Only gated unauthenticated browser requests mint signed references. Public context reads validate signature/issuer/audience/type/expiry, activation, redirect registration, and rollout. Human login atomically consumes the context before resuming the stored snapshot; replay is not-found-shaped. Social-login failure leaves it usable. Forced login requires a session created after the handoff. `/connect/app/start/$ctx` embeds the existing AuthFlow and resumes only a same-origin stored transaction; generic login is unchanged.
- **Branding**: PNG/WebP logo uploads are capped at 256 KiB and 512 × 512 pixels, decoded and re-encoded to metadata-free PNG, then stored as immutable UUID-string GridFS assets in `branding_assets`. Replacement and developer-app deletion best-effort remove the previous asset after the client update. Homepage URLs are HTTPS/public-host shape-validated and never fetched; `handoff_blurb` is bounded plain text. Name/logo/blurb/homepage changes increment `branding_revision`; only platform admins set `branding_verified_revision` against the reviewed revision. Equality adds a Verified chip, never gates an active client's name/logo. Login, checklist, and bound consent share the fixed “Secured by NyxID · destination” footer.
- **Terminal webhooks**: `app_connect_link.completed|cancelled|expired|failed` use defaulted `webhook_event_*` fields as a bounded durable outbox for both origins. Reservation freezes ids/codes, selected slugs, occurrence time, and a stable event ID; never serialize callback URLs, capabilities, or authorize parameters. An unredeemed Authorize expiry without existing consent discloses only link ID, origin, and status. Retries retain `occurred_at`; only signing timestamps change. Authorize completion dispatches after the consent/code/completion transaction commits, never at ReadyForConsent. The expiry sweep recovers unreserved terminal rows, reclaims stale cycles, and abandons exhausted deliveries. The shared developer dispatcher enforces signing and per-app admission. Check live rollout on every cycle and abandon suppressed events. Receivers deduplicate event IDs and reconcile status. No readiness subscriptions or requirement fields on `connection.expired`.
- **CLI**: `nyxid developer-app` manages requirements, handoff text, and branding; `nyxid admin app-connect` manages rollout, capability, and branding verification (syntax in Available Commands). Developer APP accepts a client UUID or exact name; `--org` uses Rule 9's resolver. Admin APP is a client ID. Reuse `AuthArgs`, saved profiles, JSON stdout, and table stderr. Pure Agent Key client binding/entrance remains outside App Connect.
