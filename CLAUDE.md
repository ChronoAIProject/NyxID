...
- **Grant decisions and delivery**: only first-party human sessions/access JWTs without an OAuth client ID can decide grants. Restricted selection uses `/auth/device/approve-agent-key`, so an old replica cannot silently issue account access. Issuance and approval commit together. Decrypt before atomically claiming `approved` -> `delivered`, retaining retryability on KMS failures. Account browser delivery atomically revokes approval bearer credentials and creates the cookie session. `/auth/device/v2/poll-web` can still return a one-time CLI handoff for a restricted v2 approval (handoff redemption and direct polling have one winner), but no first-party page calls it: browser sign-in is always an account session. Abandonment revokes only its session/child, preserving reused parent configuration and other consumers.
...
- **Web login**: the unauthenticated login page can start the browser delivery flow with an explicit click, showing both the QR payload and manual code; it never starts on mount or auto-regenerates codes. It uses only the legacy `/auth/device/request` and `/auth/device/poll-web` routes, so a browser signing itself in always requests and receives a full account session and its 8-character code stays scannable by every shipped mobile build. Restricted Agent Keys are requested only by terminal-originated flows (`nyxid login --agent-key`); never point the login page at `/auth/device/v2/*`.
