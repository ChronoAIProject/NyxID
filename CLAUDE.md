...
### 13a. App requirements and gated consent

- `oauth_authorize_contexts` stores validated authorize snapshots for five minutes with BSON dates, UUID-string IDs, and an `expires_at` TTL index. Only gated unauthenticated browser requests mint signed references; public context reads validate signature/issuer/audience/type/expiry plus current activation, redirect registration, and rollout. Human login atomically consumes the context before resuming only the stored snapshot, including consumed PAR parameters; replay is not-found-shaped. Social-login failure leaves the context usable. Forced login requires a session created after the handoff.
- OAuth branding logos are bounded PNG/WebP uploads decoded and re-encoded to metadata-free PNG, stored as immutable UUID-string GridFS assets in `branding_assets`. Replacing a logo or deleting a developer app best-effort deletes the previous asset after the durable client update. Homepage URLs are HTTPS/public-host shape-validated and never fetched. Branding updates atomically increment `branding_revision`; only platform admins set `branding_verified_revision` against the reviewed revision. Equality adds a Verified chip, never gates display of an active client's name/logo. The hosted login, checklist, and bound consent share the fixed NyxID/destination footer.
- `app_requirement_manifests` contains immutable UUID-string rows with per-client monotonic versions. Publication freezes catalog slug IDs, seeded-prefix membership, and validator versions, and advances `OauthClient.current_manifest_version` in the same MongoDB transaction. `Advise` and `Gate` publication both require the rollout and capability check.
- `app_requirement_results` stores the local evaluation's client, person, manifest version and per-requirement selection; fresh evaluations reuse the newest unexpired result when those selections are identical without extending expiry; BSON `expires_at` has a one-hour TTL. Automatic selections are not explicit user preferences. Results confer no execution grant. Shadowed selections report `Unsatisfiable`/`slug_shadowed` and remain unmet under the proxy’s personal-first precedence.
...
- `/app-requirements/status` requires an ordinary developer-app user access token, is limited to 30 requests per user per minute, and discloses only manifest services. It evaluates local authority and phase-0 evidence without credential materialization or provider I/O; `granted_to_caller` comes from the token's own service allowlist. Org candidates require `CredentialSource::Org { allowed: true }`. Disabled rows are disclosure-only and never auto-enabled; eligible no-credential services use the existing auto-provision path and appear as Included. Advise leaves authorize and consent unchanged.

- `app_connect_links` holds immutable app/person/manifest bindings and App- or Authorize-origin sessions. Page capabilities are hashed, redeemed once by the bound human Session, and cleared; resumed reads require that same subject. Org resource write access never substitutes for subject ownership. Child ConnectLinks carry parent/requirement/reauthorization/scope fields and server-derived internal return URLs. Child cancellation and parent attempt invalidation commit together; item/terminal writes compare durable attempts/revisions. Repair completion never issues tokens or changes consent. App-facing reads require both token client and subject; every route observes the rollout gate. TTL is 30 minutes plus one 15-minute extension per Met requirement, capped at two hours, with one-day terminal retention. `OauthClient.handoff_blurb` is bounded plain text; hosted destination/security text is fixed.

- App Connect Link terminal webhooks (`app_connect_link.completed|cancelled|expired|failed`) use the session's defaulted `webhook_event_*` fields as a bounded durable outbox for both origins. Reservation freezes only ids/codes, selected slugs, and a stable event ID; never serialize origin callbacks, capabilities, or authorize parameters. Authorize completion dispatches only after the consent/code/completion transaction commits, never at ReadyForConsent. The existing expiry sweep recovers unreserved terminal rows, reclaims stale cycles, and abandons exhausted deliveries; the shared developer dispatcher enforces signing and per-app admission. Check rollout on every cycle; suppressed events are abandoned. Receivers deduplicate event IDs and reconcile status. No readiness subscriptions or requirement fields on `connection.expired`.

- Authorize checks Gate manifests only for rollout-enabled capable clients, with local evidence no older than 60 seconds; `prompt=none` cannot create a session and `nyx_connect=force` opens the checklist. Consumed PAR parameters are stored server-side. Ready freezes result selections, transitions to `ReadyForConsent`, and mints a fresh 15-minute consent JWT binding result/session/subject/client/parameters. Form omissions fail closed. Code publication, consent and session completion commit in one MongoDB transaction fenced by revision. Stored service IDs are the token authority boundary; resource URIs are derived display data. Resource narrowing rejects active, accessible slug substitution and widening, including on code exchange and refresh. Inactive/tombstoned IDs remain bound for Disable/Enable; missing IDs and inaccessible active rows narrow the issued boundary instead of breaking refresh. Cancellation/failure/expiry redirects use only the validated stored URI and original state; the expiry sweep never delivers browser callbacks.
