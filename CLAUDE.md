...
- `ApiKey.allow_auto_connected_services` adds active, same-owner auto-provisioned and explicit platform-bound service IDs to restricted keys at auth-context construction; defaults to false. Management listing may idempotently provision explicit platform-key rows for an active org Member/Admin with `can_proxy()`. Org grants create org-owned auto-connected rows; grant removal reconciles those rows away. Legacy no-auth provisioning remains personal-only during membership traversal, and authentication never provisions.
