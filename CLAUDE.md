...
- **ACL and compatibility**: `ensure_can_submit` uses visibility and `org_service::resolve_owner_access`; `ensure_can_manage` gates pool mutation, worker allocation and reads, commands, and login fanout. Bundle download requires ordinary authentication; install proves manager access when it allocates the worker label. Unauthorized task/session reads are not-found-shaped. All new fields are optional or serde-defaulted. Existing rows, deployed userscripts, and old CDP workers remain valid.
...
nyxid oracle worker list <pool> | show <pool> <label>
