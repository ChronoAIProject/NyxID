...
- `/channel-bots/managed-onboarding/{platform}` (GET), `/channel-bots/managed-onboarding/{platform}/complete` (POST), `/channel-bots/{id}/reregister` (POST) -- human-session onboarding; writes rate-limited per user
...
Managed WhatsApp Embedded Signup uses adapter-owned platform-credential/onboarding/dispatcher hooks, with encrypted `platform_credentials` configured at Admin > Platform Credentials. Legacy bots default to `credential_source = "user"`; managed bots use `"platform"`, resolve the app-secret fallback on demand, and store their registration PIN encrypted. Meta Business App coexistence uses Cloud API, skips number registration, and requests contact/history sync (no historical chat import). Platform callbacks fan out through existing per-bot verification/dedup; WABA overrides also fan out because they cover every number in that WABA. No new environment variables. See the managed onboarding section in `docs/CHANNEL_BOT_RELAY.md` for prerequisites, token lifetime, and the Embedded Signup legacy flow deadline. The manual setup below describes BYO credentials.
