...
- **Worker token and bundle**: `nyx_owk_...` is 32 random bytes; only SHA-256 is stored, and create/rotate shows it once. Install and login use a local token file, environment variable, or hidden prompt and NEVER rotate implicitly. The backend embeds `worker.mjs`; manager and worker-token endpoints return source, version, SHA-256, and the exact `playwright-core` version. CLI install and worker upgrade verify SHA-256. Worker upgrade refreshes the dependency with a bounded npm command before it replaces the source. launchd or systemd restarts the worker after upgrade.
