...
- `app_requirement_results` stores the local evaluation's client, person, manifest version and per-requirement selection; fresh evaluations reuse the newest unexpired result when those selections are identical without extending expiry; BSON `expires_at` has a one-hour TTL. Automatic selections are not explicit user preferences. Results confer no execution grant.
