...
- Catalog `platform_key` uses the existing encrypted master credential and live public/restricted polymorphic-owner grants. Every platform-bound resolver must call the shared ACL before decryption; no anonymous grant, no node routing or user-controlled destination. Gateway-URL providers cannot enable platform keys. Restricted ACLs batch memberships/active owners; status reuses one membership snapshot per request. Legacy absent configs preserve public internal master-key behavior. `UserService.credential_binding` is optional; `POST/PUT /keys` and hosted/MCP flows use `use_platform_key`. Switching to platform retains the user's key. Automatic grants reconcile personal/org rows; scoped agent auto-connected grants also include explicit platform bindings.
