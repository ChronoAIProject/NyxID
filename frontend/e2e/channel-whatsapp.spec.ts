...
import { channelPlatforms, larkSetupInstructions } from "./channel-platform-fixtures";

for (const viewport of [
  { width: 1440, height: 1000 },
  { width: 390, height: 844 },
]) {
  test(`WhatsApp registration and one-time Verify Token at ${String(viewport.width)}px`, async ({
    page,
  }) => {
    ...
    const bot = {
      ...
      user_id: "test-user",
      webhook_url: "https://nyxid.example/api/v1/webhooks/channel/whatsapp/whatsapp-e2e",
      webhook_secret_label: "Verify Token",
      setup_instructions: [
        "Enter the Callback URL and Verify Token in Meta App Dashboard.",
    ...
    for (const platform of ["Lark", "Feishu"]) {
      ...
      await expect(dialog.getByText("Setup instructions", { exact: true })).toBeVisible();
      await expect(dialog.getByText(larkSetupInstructions, { exact: true })).toBeVisible();
    }
    await dialog.getByRole("combobox").last().click();
    ...
    await dialog
      .getByLabel("WhatsApp Business Account ID (optional)", { exact: true })
      .fill(bot.waba_id);
    await expect(dialog.getByRole("button", { name: "Add Bot", exact: true })).toBeEnabled();
    await dialog.screenshot({
      path: test.info().outputPath(`nyxid-whatsapp-form-${String(viewport.width)}.png`),
    });
    await dialog.getByRole("button", { name: "Add Bot", exact: true }).click();
    await expect(dialog.getByText(secret, { exact: true })).toBeVisible();
    await expect(
      dialog.getByRole("button", { name: "Copy Verify Token", exact: true }),
    ).toBeVisible();
    expect(registration).toEqual({
      platform: "whatsapp",
      label: bot.label,
      bot_token: "e2e-access-token",
      phone_number_id: "123456",
      waba_id: "654321",
      app_secret: "e2e-app-secret",
    });
    await dialog.screenshot({
      path: test.info().outputPath(`nyxid-whatsapp-created-${String(viewport.width)}.png`),
    });
    ...
    await expect(
      page.getByText("Saving stores these credentials immediately", { exact: false }),
    ).toBeVisible();
    ...
    expect(rotation).toEqual({
      bot_token: "replacement-access-token",
      app_secret: "replacement-app-secret",
    });
    await page.screenshot({
      path: test.info().outputPath(`nyxid-whatsapp-detail-${String(viewport.width)}.png`),
      fullPage: true,
    });
