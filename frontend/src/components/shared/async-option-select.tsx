import { useEffect, useId, useImperativeHandle, useLayoutEffect, useRef, useState, type ComponentProps } from "react";
...
import type { OptionItem, OptionsContext, OptionSet } from "@/types/options";
...
} & Pick<ComponentProps<"input">, "id" | "onBlur" | "aria-describedby" | "aria-invalid" | "ref">;

type Choice = { kind: "branch" | "leaf"; value: string; label?: string; disabled?: boolean };
...
function suggestions(items: OptionItem[], draft: string, delimiter?: string): Choice[] {
  ...
  for (const item of items) {
    const fullMatch = item.value.toLowerCase().includes(draft.toLowerCase()) || item.label.toLowerCase().includes(draft.toLowerCase());
    let choice: Choice | undefined;
    if (delimiter && item.value.toLowerCase().startsWith(prefix.toLowerCase())) {
      ...
      if (next >= 0 && rest.slice(0, next).toLowerCase().startsWith(fragment)) {
        choice = { kind: "branch", value: item.value.slice(0, prefix.length + next + delimiter.length) };
      } else if (fullMatch) {
        choice = { kind: "leaf", value: item.value, label: item.label, disabled: item.disabled };
      }
...
function replaceSelection(base: readonly string[], original: string | undefined, additions: string[]) {
  return [...new Set(original === undefined
    ? [...base, ...additions]
    : base.flatMap((value) => value === original ? additions : [value]))];
}

function OptionSelection({ optionSet, context, value, onChange, label, disabled, allowCustom = false, delimiter, ref: forwardedRef, ...inputProps }: Props) {
  ...
  const editing = pending && JSON.stringify(value) === JSON.stringify(pending.expected) ? pending : null;
  ...
  useLayoutEffect(() => {
    ...
    if (target.value !== undefined) {
      const button = [...(field.current?.querySelectorAll<HTMLButtonElement>("[data-scope-edit]") ?? [])].find((entry) => entry.dataset.scopeEdit === target.value);
      button?.focus();
    } else {
      input.current?.focus();
  ...
  const query = useOptions(optionSet, context, search);
  ...
  const pages = query.isError && !query.retainPartialData ? [] : (query.data?.pages ?? []);
  const items = [...new Map(pages.flatMap((page) => page.items).map((item) => [item.value, item])).values()];
  ...
  const choices = suggestions(items.filter((item) => !selected.includes(item.value)), draft, delimiter);
  const activeIndex = choices.findIndex((choice) => choiceKey(choice) === active);
  useEffect(() => {
    if (activeIndex >= 0) menu.current?.querySelector(`[data-option-index="${activeIndex}"]`)?.scrollIntoView?.({ block: "nearest" });
  }, [activeIndex]);

  function updateDraft(text: string, branch = false) {
    if (disabled) return;
    const base = editing?.base ?? [...value];
    const expected = !branch && allowCustom
  ...
  function commitDraft() {
    if (!disabled && allowCustom && editing && !editing.branch) finish(draft.split(/\s+/).filter(Boolean));
  }
  function cancelEdit() {
  ...
  function edit(valueToEdit: string) {
    ...
    setPending({ base, original: valueToEdit, text: valueToEdit, branch: false, expected: [...value] });
    setActive(null);
    setOpen(true);
  }
  ...
  function navigate(direction: number) {
    ...
    for (let count = 0; count < choices.length; count++) {
      ...
      if (!choices[next]!.disabled) { setActive(choiceKey(choices[next]!)); break; }
    }
  }
  function leaveWidget(target: EventTarget | null) {
    if (!(target instanceof Node) || (!field.current?.contains(target) && !menu.current?.contains(target))) setOpen(false);
  }
  ...
  const editor = <input {...inputProps} ref={input}
    ...
    onPaste={(event) => {
      ...
      if (/\s/.test(pasted)) {
        ...
        finish((draft.slice(0, start) + pasted + draft.slice(end)).split(/\s+/).filter(Boolean));
      }
    }}
    onKeyDown={(event) => {
      if (event.nativeEvent.isComposing) return;
      if (event.key === "ArrowDown" || event.key === "ArrowUp") { event.preventDefault(); navigate(event.key === "ArrowDown" ? 1 : -1); }
      else if (event.key === "Enter") { event.preventDefault(); if (open && activeIndex >= 0) choose(choices[activeIndex]!); else commitDraft(); }
      else if (event.key === "Escape" && (open || original !== undefined)) {
        event.preventDefault(); event.stopPropagation();
        if (original !== undefined) cancelEdit(); else { setOpen(false); setActive(null); }
      }
    }} />;
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    ...
    <PopoverContent role="presentation" ref={menu} onBlur={(event) => leaveWidget(event.relatedTarget)} align="start" className="w-[var(--radix-popover-trigger-width)] min-w-56 max-w-[calc(100vw-2rem)] p-1.5"
      ...
      {original !== undefined && <p className="border-b px-2 py-2 text-xs text-muted-foreground">Editing scope. Enter saves; Escape cancels.</p>}
      {query.isFetching && <p role="status" className="px-3 py-2 text-xs text-muted-foreground">{pages.length ? "Loading remaining suggestions…" : "Loading suggestions…"}</p>}
      {query.isError && !query.versionChanged && <div role="alert" className="px-3 py-2 text-xs"><p className="text-destructive">{pages.length ? "Some suggestions could not load." : "Suggestions unavailable."}{allowCustom ? " Custom values still work." : ""}</p><Button type="button" variant="ghost" size="sm" onPointerDown={(event) => event.preventDefault()} onClick={() => void (query.isFetchNextPageError ? query.fetchNextPage() : query.refetch())}>Retry suggestions</Button></div>}
      ...
      {!choices.length && !query.isFetching && query.isSuccess && <p role="status" className="px-3 py-2 text-xs text-muted-foreground">No matching suggestions.</p>}
      {allowCustom && draft.trim() && !editing?.branch && <p className="border-t px-2 py-2 text-xs text-muted-foreground">Press Enter to finish this scope.</p>}
    </PopoverContent>
  </Popover>;
}
