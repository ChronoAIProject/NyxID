// Notion mark, in the catalog's monochrome style. The source artwork's
// `.cls-1 { fill-rule: evenodd }` is inlined on the path; `fill` stays
// `currentColor` so the tile inherits the catalog's 2-tone treatment.
export default function ApiNotionIcon({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 122.88 128.1"
      fill="currentColor"
      aria-hidden="true"
      data-slug="api-notion"
      className={className}
    >
      <path
        fillRule="evenodd"
