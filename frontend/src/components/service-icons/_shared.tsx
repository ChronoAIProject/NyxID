...
/**
 * Firecrawl flame mark. Supplied by Calvin (50×72 canvas, single path).
 * Original SVG uses `fill="white"`; flattened to `currentColor` per the
 * 2-tone rule so the tile matches the rest of the monochrome grid.
 * Simple Icons has no Firecrawl entry at the time of writing.
 */
export function FirecrawlGlyph(props: GlyphProps) {
  return (
    <svg viewBox="0 0 50 72" fill="currentColor" aria-hidden="true" {...props}>
 ...
 */
export function LarkFamilyGlyph({
  variant = "solid",
  ...props
}: GlyphProps & { readonly variant?: "solid" | "outline" }) {
  // Path data lifted from the user-supplied Lark brand SVG (`viewBox="0 0 792
  // 792"`). The three colored panels (upper wing, lower body, accent) are
  // merged into `currentColor` and the viewBox is cropped to the bird's
  // bounding box so it fills the tile without dead margin.
  //
  // Lark and Feishu share the same shape (same underlying product); the
  // `variant` prop is how the four Lark-family tiles distinguish themselves
  // visually while preserving the shared brand mark:
  //   - Lark          → solid (dominant, primary international brand)
  //   - Feishu        → outline (secondary, China-market variant)
  //   - Lark Bot      → solid + Bot badge
  //   - Feishu Bot    → outline + Bot badge
  const fill = variant === "outline" ? "none" : "currentColor";
  const stroke = variant === "outline" ? "currentColor" : "none";
  // ViewBox is 660 units wide; ~30-unit stroke ≈ 4.5% width — visible +
