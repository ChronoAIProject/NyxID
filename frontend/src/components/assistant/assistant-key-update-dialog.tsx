...
const keyResourceSchema = z
  ...
  .strict();
const assistantKeyUpdateResponseSchema = z
  .object({
...
export type AssistantKeyUpdateParams = z.infer<
  typeof keyUpdateActionParamsSchema
>;

export function AssistantKeyUpdateDialog({
  open,
  onOpenChange,
  actionRequestId,
  params,
  ...
  async function verifyUpdate(
    keyId: string,
    expected: AssistantKeyUpdateParams,
  ): Promise<void> {
    ...
    try {
      ...
      if (expected.platform && snapshot.platform !== expected.platform) {
        throw new Error("NyxID key verification did not match this action.");
      }
      setVerified(true);
      setError(null);
      ...
      setError(
        errorMessage(caught, "NyxID could not verify the updated API key."),
      );
    } finally {
      verificationRef.current = false;
      setVerifying(false);
  ...
  async function updateKey() {
    ...
    try {
      ...
      const response = assistantKeyUpdateResponseSchema.parse(
        await api.post<unknown>("/assistant/actions/keys/update", {
          actionRequestId,
      ...
      await verifyUpdate(response.resource.keyId, expected);
    } catch (caught) {
      setError(errorMessage(caught, "NyxID could not update this key."));
    } finally {
      submittingRef.current = false;
      setSubmitting(false);
    }
  ...
  return (
    ...
    >
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {resultKeyId ? "API key updated" : "Update API key"}
          </DialogTitle>
          <DialogDescription>
            {resultKeyId
