...
export function AssistantAccountRevokeConsentDialog({
  ...
  function close() { submittingRef.current = false; setSubmitting(false); setVerifiedUserId(null); setError(null); onOpenChange(false); }
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && close()}>
      <DialogContent>
        ...
        {!verifiedUserId ? <div className="flex items-center justify-between border-y border-border py-4 text-[12px]"><span className="text-muted-foreground">Client</span><Badge variant="secondary" className="font-mono">{params.clientId}</Badge></div> : null}
        {error ? <p role="alert" className="text-[11px] text-destructive">{error}</p> : null}
        <DialogFooter>{verifiedUserId ? <Button type="button" variant="primary" onClick={() => { onComplete(verifiedUserId); close(); }}>Done</Button> : <><Button type="button" variant="outline" onClick={close}>Cancel</Button><Button type="button" variant="destructive" isLoading={submitting} disabled={submitting} onClick={() => void submit()}>Revoke consent</Button></>}</DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
