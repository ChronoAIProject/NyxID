...
export function useLoginCodeOptions() {
  return useMutation({
    gcTime: 0,
    mutationFn: async () =>
      agentKeyOptionsSchema.parse(
        await api.post("/auth/login-code/options", {}),
      ),
  });
}
...
export function useLoginCodeStatus(id: string) {
  return useQuery({
    queryKey: ["login-code", id],
    gcTime: 0,
    queryFn: async () =>
      loginCodeStatusSchema.parse(
        await api.get(`/auth/login-code/${encodeURIComponent(id)}`),
      ),
...
export function useLoginCodeAction(id: string) {
  return useMutation({
    gcTime: 0,
    mutationFn: (action: "cancel" | "revoke") =>
      action === "cancel"
