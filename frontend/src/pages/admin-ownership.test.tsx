...
import { AdminOwnershipPage, OwnershipTransferDialog } from "./admin-ownership";
...
vi.mock("@/hooks/use-ownership-transfers", () => ({
  useOwnershipResources: (...args: unknown[]) => {
    mock.resources(...args);
    return { data: { items: [resource], next_offset: null } };
  },
...
it("does not load transfer inventory or show actions for operators", () => {
  mock.user = { id: "operator", is_admin: false, role: "operator" };
  render(<AdminOwnershipPage />);
  expect(
    screen.getByText("NyxID admin access is required to transfer ownership."),
  ).toBeInTheDocument();
  expect(mock.resources).toHaveBeenCalledWith("service", "", 0, false);
  expect(
