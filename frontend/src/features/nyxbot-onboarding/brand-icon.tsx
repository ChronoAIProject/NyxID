...
export function BrandIcon({ brand }: { readonly brand: string }) {
  if (brand === "google")
    return (
      <svg viewBox="0 0 24 24" className="nb-brand-icon" aria-hidden="true">
        <path
  ...
  if (brand === "apple")
    return (
      <span className="nb-brand-icon nb-apple">
  ...
  if (brand === "github")
    return (
      <span className="nb-brand-icon nb-github">
        <ServiceIcon slug="api-github" size="sm" className="!text-white" />
      </span>
    );
  return (
    <ServiceIcon
