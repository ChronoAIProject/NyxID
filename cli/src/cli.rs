...
pub struct LoginArgs {
    ...
    pub device: bool,
    /// Use the local browser-callback login: opens the web console and completes with a full account session, no code entry
    #[arg(long, conflicts_with_all = ["password", "device", "agent_key", "code", "no_wait"])]
    pub callback: bool,
    /// Copy the user code before browser opening (best-effort); --output json and --no-wait only print it, without copying or opening a browser
    #[arg(short = 'c', long)]
    pub clipboard: bool,
