...
pub struct AppState {
    ...
    /// Per-IP limiter for authorize context reads and login continuation (30/min).
    pub authorize_context_limiter: mw::rate_limit::SharedPerIpRateLimiter,
    /// Per-user limiter for `GET /api/v1/app-requirements/status` (30/min).
    pub app_requirements_status_limiter: mw::rate_limit::SharedPerKeyRateLimiter,
    /// Per-user limiter for `POST /api/v1/keys/{id}/validate` (10/min).
...
async fn main() {
    ...
    let state = AppState {
        ...
        authorize_context_limiter: mw::rate_limit::create_per_ip_rate_limiter(
            db.clone(),
            "authorize_context",
            30,
            60,
        ),
        app_requirements_status_limiter: mw::rate_limit::create_per_key_rate_limiter(
            db.clone(),
