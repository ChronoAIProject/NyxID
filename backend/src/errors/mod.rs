...
pub enum AppError {
    ...
    #[error(
        "workspace_destinations_not_activated (12100): Workspace editor destinations are not activated. Ask the NyxID operator to enable GOOGLE_WORKSPACE_MULTI_ORIGIN_ENABLED after upgrading backend readers and node agents, then retry."
    )]
...
impl AppError {
    ...
    pub(crate) fn error_code(&self) -> u32 {
        match self {
            ...
            Self::WorkspaceDestinationsNotActivated => 12100,
