...
Managed Aurinko bots resolve the live shared AI Service connection and the current platform signing secret for each operation. Signed setup challenges remain outside the connection claim so a subscription request can receive its synchronous validation callback. The stored signing fingerprint comes from the same encrypted secret snapshot used to verify the signature; setup checks that it is still current before reporting success. Connection mutations acquire the connection claim before bot claims, and reconnect advances the bot generation strictly even within one millisecond. The existing metadata-only receipts and irreversible send barriers remain authoritative. Aurinko may poll or subscribe to its upstream provider before sending these webhooks; the channel does not promise uniform instantaneous delivery.
