...
| `custom_filters` | JSON string | --        | URL-encoded per-filter free text, matched as a `contains` and OR'd with that filter's selected options; see the custom-text semantics below |
...
`custom_filters` is a URL-encoded JSON object mapping a filter key to its free-text values, for example `{"client_type":["machine"],"creator_type":["d0d7b72a"]}`. Only filters backed by a single string column accept custom text: `client_type` (matched against `client_type`), `creator_type` (matched against `created_by`), and `scope` (matched against `allowed_scopes`). `is_active` is a boolean, `broker` is derived from several fields, and `created_at` is a date, so none of them has a column a substring could match; supplying custom text for them returns `400 validation_error`. Each field in `filter_options.fields` advertises its capability as `supports_custom_text`.

Each custom value is a literal case-insensitive `contains` match; regular-expression metacharacters have no special meaning. A filter's custom values are combined with OR against that same filter's selected options, so `client_type=public&custom_filters={"client_type":["machine"]}` matches clients whose type is `public` **or** contains `machine`. This widens one filter rather than narrowing the result set, which is what distinguishes it from `search_filters` (a separate AND group). A filter carrying only custom text and no selected options still applies. The object permits at most five filters, eight values per filter, and 32 values in total; every value is trimmed, must be non-empty, and may contain at most 256 Unicode characters. Unknown filters, empty arrays or values, malformed JSON, and exceeded limits return `400 validation_error`.
...
Platform-bound connections validate the current catalog credential using server transport. A retained personal key or legacy node binding does not change that selection. Platform credential rotation invalidates prior observations, and live platform grants remain required.
...
Provider `Retry-After` delays are honored. Admission failures use error 12109
(429), missing attempts or an unavailable wait use 12108 (503), and locally rejected
validation uses 12107 (422), including a disabled service. The Check connection
...
A manifest has at most 25 requirements with unique stable IDs matching `[a-z0-9_-]{1,32}`. Each resolves to 1–25 active, user-connectable catalog slugs. `any_of_catalog_prefix` (at most 128 characters) expands active seeded, non-provider slugs at publication and freezes that membership; later seeds enter only on republish. Unknown slugs, catalog tags, explicitly listed provider-category or inactive rows, unknown profiles, and profiles applying to none of a requirement's slugs are rejected with `12100 AppRequirementsInvalid` (400). `enforcement` accepts `advise` or `gate`. An empty manifest clears the current requirements by publishing a new version.
...
`12110 AppConnectResultMismatch`, including when `allow_all_services=true`.
