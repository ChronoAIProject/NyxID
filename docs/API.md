...
**Assistant Forward Token (transitional, assistant mount only):** Not a new identity mode. On the `/api/v1/assistant/*` pass-through, a browser session authenticates with a cookie and therefore carries no bearer for `forward_access_token` to forward. While the `aevatar` service row has `forward_access_token: true`, NyxID mints an outbound-only access token for the session user and sends it as `Authorization: Bearer` alongside the standard identity headers above (which flow unchanged). The token has a 5-minute TTL (`JWT_ASSISTANT_FORWARD_TTL_SECS`) and carries `assistant_forward: true`, which NyxID's shared token validator rejects — a copy leaked from the downstream cannot be replayed against any NyxID API. Unlike identity/delegation injection (which log and continue on generation failure), a mint failure fails the assistant request: the bridge token is required authentication for the downstream today, and an unauthenticated attempt would only produce a confusing downstream 401. Flipping the row to `forward_access_token: false` (the identity-token rollout) retires the bridge with no code change.
