...
5. **Long holds.** `wait_signal`'s timeout is capped at exactly 24 h (86,400,000 ms, verified). Holds longer than that use a **re-entrant wait loop** — checkpoint, wait ≤ 24 h, re-check the deadline, wait again — with `self_reschedule` as the fallback if looped waits prove awkward.
...
- **One gate in front of every master-credential decrypt**, enforced by the type system: a single authorization function is the only constructor of a decryptable catalog credential, evaluated against the real effective actor (with consent checks for private rows that work even when no per-user service record exists). The one resolution path with no real user behind it — the server-chosen/admin path — is handled by prohibition: it may never serve a private credentialed row, and no synthetic actor can be constructed to pretend otherwise.
...
**Assumptions to retire:** Duffel component-key/3DS/`@duffel/components` contracts; payment-body token path; passenger-email field path; hold-window distribution and holdable-fare share; Aevatar signal surface; `approval.node_id` semantics; workflow composition details; Duffel idempotency-key absence (nothing depends on it).
