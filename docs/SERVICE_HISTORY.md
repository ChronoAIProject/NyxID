...
Personal owners and current organization admins may read history only within their current service scope. The owner must remain active, and API-key service restrictions apply in addition to owner permissions. Members, viewers, revoked admins, and admins outside the service scope receive no authorship metadata. Ordinary service visibility does not imply history access. Existing route credential gates remain in place: the current human-only keys/endpoints routes reject API keys before handlers; handler-level service/node restrictions are additional defense in depth, not broader route authorization. Archived discovery uses the same gates before returning any UUID or display snapshot.
...
| --- | --- | --- |
| `user_service_service`, `user_endpoint_service`, `user_api_key_service` | Direct creation, endpoint/configuration changes, credential replacement, routing, SSH, Enable/Disable, shared backing changes | Verified request; real ciphertext replacement is names-only. Saving equal non-secret config is silent. |
...
| `assistant_action_effects_services` / endpoint helpers | Session-aware configuration, credential and routing commits | Existing receipt group; database retries preserve journal identity. |
...
| `db` migrations, `org_service`, `cleanup_cli` | Service-instance migrations and physical removal; cleanup includes disabled references | No history cascade deletion. Existing records are not assigned a historical creator. |
