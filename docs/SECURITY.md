...
| **Exact catalog representation binding** | Delegated discovery v3 publishes `exact_view_digest` over its sorted generic-free service/operation projection, including each producer-owned `ServiceEndpoint.operation_generation`. New exact approvals persist and revalidate that value and generation in addition to the legacy whole-catalog `catalog_digest`; visibility remains separate from generic-operation execution eligibility. Missing/non-positive producer generations are not published. |
| **Delegated exact-approval authority** | Delegated exact-service requests must carry `mcp:catalog:read` on create, status and redeem (`403 delegated_catalog_scope_required` otherwise), closing the path where a proxy-only delegated token reached these routes without live catalog-grant revalidation. Delegated callers must also supply `exact_view_digest` at create/redeem and the matching discovery `operation_generation` at create; omitted, empty, noncanonical, or mismatched fences fail closed. Nondelegated callers may omit the additive discovery fields, but NyxID still persists the live producer generation and rejects supplied mismatches. |
| **Exact-service execution-authority binding** | Create persists an explicit `{ projection_version, digest }` for `nyxid-exact-execution-authority.v2` over the resolved execution inputs: destination URL, auth method, credential id+epoch, identity injection, default headers, proxy operation policy, configured node bindings, `service_category`, `requires_user_credential`, and an execution-only digest of `token_exchange_config`. Observe and redeem share the ordered live evaluator `catalog/exact generation -> policy -> read-only execution authority`. Observe is transient and read-only; redeem claims first and durably records terminal revalidation results. Credential materialization runs only after those gates and is followed by another execution-authority comparison before effect. Mismatch is HTTP 200 `state: drifted` / `failure_code: execution_authority_drift` with zero provider effect. Missing, v1, or unknown projection bindings fail closed and never fall back to the legacy digest. New rows put a fail-closed marker in that legacy slot, preventing v1 replicas from executing v2-only authority; operators must drain v1 replicas and reconcile outstanding v1 approvals before v2 issuance. `credential_epoch` bumps only on user-initiated credential replacement, not background token refresh. Content-addressed fields accept A-to-B-to-A; credential epochs do not. An `executing` claim left stale for 15 minutes by a crash or terminal-write failure becomes durable `failed/provider_outcome_unknown` on redeem retry and is never automatically replayed. |
