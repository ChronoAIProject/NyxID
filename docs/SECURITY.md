...
| **Delegated exact-approval authority** | Delegated exact-service requests must carry `mcp:catalog:read` on create, status and redeem (`403 delegated_catalog_scope_required` otherwise), closing the path where a proxy-only delegated token reached these routes without live catalog-grant revalidation. Delegated callers must also supply `exact_view_digest` at create and redeem; omitted, empty, noncanonical and mismatched values all fail closed. Nondelegated callers keep the prior optional-fence behaviour. |
| **Exact-service execution-authority binding** | Create persists a producer-owned digest of the resolved execution inputs (destination URL, auth method, credential id+epoch, identity injection, default headers, proxy operation policy, configured node bindings). Redeem recomputes that digest from the same resolution object it then executes, so an owner cannot retarget a pending approval via `PUT /keys/{id}` while shape digests stay still. Mismatch is HTTP 200 `state: drifted` / `failure_code: execution_authority_drift` with zero provider effect. `credential_epoch` bumps only on user-initiated credential replacement, not background token refresh. Content-addressed fields accept A→B→A; credential epochs do not. Cache age on this path is zero (direct MongoDB reads). The current implementation keeps `operation_generation` as caller bookkeeping and proposes live `endpoint_contract_digest` plus this execution-authority digest as equivalent producer freshness evidence; eanz17 sign-off on that interpretation remains outstanding. Legacy rows with `execution_authority_digest: None` skip the gate until approval expiry. |
