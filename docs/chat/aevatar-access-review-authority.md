...
| --- | --- |
| Atomic consent merge | Add `consent_service::merge_consent_services_atomic` in `backend/src/services/consent_service.rs`. First run a non-upserting aggregation-pipeline `update_one` keyed by `(user_id, client_id)`. Set-union the requested service ID and required scopes while preserving all existing scopes, IDs, `allow_all_services`, grant identity, and unrelated fields. If `matched_count == 0`, insert a new `Consent` with a UUID v4 `_id`. On a concurrent first-approval E11000, retry the same non-upserting merge once against the winning row. No pipeline writes or synthesizes `_id`. Never call `grant_consent_with_services`. `grant_consent_internal` replaces the full row at lines 71-100, and the OAuth consent page in `backend/src/handlers/oauth.rs:807-836` still calls that replacement path; online `validate_live_grant` enforcement makes a capability that exceeds a later replacement fail closed. The unique `(user_id, client_id)` index is at `backend/src/db.rs:911-920`. |
