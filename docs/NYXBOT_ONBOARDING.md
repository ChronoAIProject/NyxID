...
- [ ] **Browser authentication for Aevatar registration:** the supplied endpoint is deployed, but it requires `Authorization: Bearer` and a resolved scope. Current NyxID onboarding uses an HttpOnly browser session; the generic proxy forwards a caller Bearer only when one exists. Aevatar's current channel UI obtains its own OIDC access token. Confirm/provide the supported browser-session forwarding contract before production registration can be considered integrated. Do not copy tokens from another application, mint CLI credentials for the web flow, or change the global proxy's authentication policy. The owner was asked for any existing bridge; the frontend currently preserves the NyxID session on upstream 401/403 and shows a safe registration-auth error.
...
- Telegram identity lookup, bot-name labeling, Aevatar registration and readback are implemented against the existing contracts, with isolated API fixtures in tests. The browser-session-to-Aevatar Bearer handoff remains a deployment/integration dependency; this implementation does not claim a successful production registration. Chat linking displays an unavailable state; code copying and chat opening remain disabled because no real code/destination contract exists.
