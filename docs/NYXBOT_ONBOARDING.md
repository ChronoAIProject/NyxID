...
- Google list (`GET /keys`), catalog (`GET /catalog/api-google`) and stored connection (`GET /keys/{id}`) queries belong exclusively to Data source and unmount when leaving that step. Plain Channel and Link URLs do not check Google permissions or redirect based on Google query errors; visiting those pages does not claim a Google grant. Channel makes no business requests on mount, reload or focus. Its submission runs Telegram verification, required Aevatar authorization and registration. Link chat retains its own bot/registration status reads. Global NyxID session checks remain in place.
