...
- Implemented inside the existing React frontend at `/onboarding`, outside the dashboard's unrelated AI Services onboarding takeover. TanStack Router search state selects three active pages:
  ...
  | Registration result (after setup) | `/onboarding?step=success` |
  | Legacy link URL | `/onboarding?step=link` (redirects to Channel) |
