...
- [ ] **Google platform readiness:** deploy the shared Google allowlist extension for `drive.file` and `calendar` included in this branch. The selected hosted `api-google` provider currently requires user-provided OAuth app credentials: configure its platform Client ID/Secret and platform-capable credential mode, together with the service OAuth callback, enabled Drive/Calendar APIs and Google's consent-screen verification or test-user access. Google account-login credentials alone do not configure the service connector. An existing authorized personal BYO connection can be reused; this onboarding does not ask end users to create a Google OAuth application.
