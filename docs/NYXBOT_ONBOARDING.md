...
- A bare `/onboarding` visit starts at Account, including for signed-in users. All four sign-in methods remain visible; an existing session additionally offers **Continue as [account name]**. Account continuation resumes Channel or the saved Link chat page after checking real grants. Explicit `step=source` stays on Data source, even with existing authorization; Continue resumes Channel or the saved Link chat page. Back follows Link chat → Channel → Data source → Account and preserves the account session.
- Account and Google service OAuth returns now target `step=channel` (an already signed-in account with a saved bot can return to `step=link`). Legacy `step=source` or step-less callbacks with `provider_status=success|error` or `status` remain supported. After verifying effective grants, successful callbacks resolve to a concrete step and remove consumed callback flags. Initial session/grant checks show a neutral loading screen. Unauthenticated downstream URLs replace with Account; missing grants or query failures replace with Data source; Link chat without a stored bot reference replaces with Channel. A URL cannot supply a bot ID, session or completion claim. An explicit Source URL remains there when a pending grant becomes active, until Continue. Data consent is separate from identity sign-in, and OAuth is never initiated on mount.
...
The owner completed Google consent, and the local page confirmed both Workspace permissions from the hosted connection. Hosted callbacks still return to the configured production frontend, which does not deploy this onboarding route. The current initiation requests `/onboarding?step=channel`; the earlier `/onboarding?step=source&provider_status=success` form remains compatible. Returning to local `?step=channel` resumes Channel after grant checks; an explicit local `?step=source` now stays on Data source. The hosted-to-local redirect limitation remains a separate deployment concern.
