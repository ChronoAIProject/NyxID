...
- Aevatar returns `202` with `status: "accepted"`, `registration_id` and `nyx_channel_bot_id`. Store these as separate resource references. The successful Telegram identity response also supplies `username`, which is converted to `https://t.me/<username>` and shown inline on step 3 while the connection form remains available for additional bots. Clicking that link asks the user's Telegram client to open the bot conversation; actual chat delivery still depends on platform support. No pairing-code request is made because that API is not available yet.
