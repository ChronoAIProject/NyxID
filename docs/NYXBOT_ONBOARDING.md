...
- A direct visit starts at Account, including for signed-in users. All four sign-in methods remain visible; an existing authenticated session additionally offers an explicit **Continue as [account name]** action. After account confirmation, verify the Google grants and automatically enter Channel (or restore the saved channel) when both permissions are already effective. A completed Data source step does not require another Continue click. Explicit Back from Channel keeps Data source visible for review; Back from Data source preserves the account session.
- Authentication and service OAuth return URLs use `step=source` to resume the data authorization check after the external flow. Service callbacks use `provider_status=success|error`; the earlier `status` hint remains compatible. Initial session restoration and unresolved grant queries show a neutral loading screen, without prematurely rendering Account, Data source or Channel. After successful grant verification the page automatically advances to Channel, including when a pending connection becomes authorized while the page is open. Missing permissions and query failures show Data source. Explicit Back and an in-progress Google consent request retain the data-source view. These hints never bypass authentication or grant checks, and `step=channel|complete` is rejected. Data source displays the real account name (email fallback), explains Drive/Calendar consent separately from NyxID sign-in, and does not initiate OAuth on mount or treat account login as data access.
- Telegram uses `useAppForm` and the shared token limits with feature-localized syntax validation. On **Connect channel**, call Telegram `getMe` with the entered token, require a bot identity and `first_name`, replace whitespace with underscores and append one `_nyxid_bot` suffix: `Test01` becomes `Test01_nyxid_bot`. Bound the whole label to NyxID's 200-byte UTF-8 limit. Submit `{ platform: "telegram", bot_token, label, webhook_base_url: "https://aevatar-console-backend-api.aevatar.ai" }` to the existing Aevatar `POST /api/channels/registrations` through the NyxID `/api/v1/proxy/s/aevatar` transport. Aevatar owns creation of the NyxID bot and relay route; the frontend does not separately post to `/channel-bots`. Telegram rejection stops before registration. Both requests share one pending state and duplicate-submit guard; errors never echo token-bearing provider messages.
...
The Channel screen follows the owner's 10 September screenshot labeled **M03 / Web / 3. Customer channel**. This revision uses that supplied image; its exact Figma node was not independently re-read. It reuses the Data source setup shell: Nyxbot brand header, completed Account/Data source indicators, current Channel indicator, left-aligned heading/subtitle and Telegram guide-chat note. Channel cards, numbered BotFather instructions, customer-token field, lock note and bottom Back/Connect actions follow the reference. The owner's subsequent constraints remain authoritative: WhatsApp is disabled and all four cards have equal width/height, including the unavailable row. Existing feature locale/theme state and token validation still apply.
...
- Telegram identity lookup, suffixed bot-name labeling, Aevatar registration and readback use the existing contracts. The browser-session-to-Aevatar Bearer handoff has been verified against the hosted services; a complete production bot registration/webhook setup remains to be tested with the owner's bot. The current integration pins the supplied production Aevatar/NyxID environment; a different environment needs its matching OAuth client and registered callback. Chat linking displays an unavailable state; code copying and chat opening remain disabled because no real code/destination contract exists.
...
- No new OAuth consent, real Telegram registration, or paid Meta activation was performed by the agent. The authorization repair reused the existing browser session and existing Aevatar consent, and obtained a fresh OAuth access token through the deployed endpoints. Automated tests use isolated API responses; live validation is described below.
...
The owner subsequently completed Google consent, and the local page confirmed both Workspace permissions from the hosted connection. The hosted callback returns to its configured frontend at `https://nyx.chrono-ai.fun/onboarding?step=source&provider_status=success`; that deployment does not yet contain this onboarding route and displays 404. Returning to the local `/onboarding?step=source` rechecks the saved grants. The automatic-advance revision now enters Channel after that check without a second Continue click. This was verified in the owner's existing browser with the real authorized account; explicit Back stayed on Data source and Continue returned to Channel. The hosted-to-local redirect limitation remains a separate deployment concern.
