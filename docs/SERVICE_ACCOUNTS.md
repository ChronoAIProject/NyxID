...
Only grant-listed catalog services appear. The IDs in this API are catalog `DownstreamService` UUIDs; they are not `/keys` or `UserService` instance IDs. A Curation token should use this route instead of `GET /api/v1/keys/{user_service_id}`, which is intentionally rejected for service accounts. Each skill response contains `service_id`, `recommended_skills`, optional `recommended_skill_refs`, `skills_revision`, and the separately versioned `skills_manifest_digest`. Disallowed service IDs return 404 without disclosing their content or history. API keys, delegated/relay tokens, and human session/access tokens cannot use this router.
...
contract capability is designed and deployed; do not open `/keys`, `/mcp/config`,
or general proxy access to compensate.
