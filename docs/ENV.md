...
Credit benefits use three collections: `credit_grants` stores one attributable row per recipient, `usage_allowances` stores recurring free-unit definitions, and `usage_allowance_periods` stores each owner's consumption and reservations for a UTC window. Platform admins create/revoke grants and create/update/disable allowances under `/api/v1/admin/credits`; operators may read those admin endpoints but cannot mutate them. Flagged users read active balances from `GET /api/v1/billing/grants` and `GET /api/v1/billing/allowances`; an org admin may pass `owner_id` to read an organization billing owner. "All users" grants snapshot the active person and organization owners at issuance, while "all users" allowances apply dynamically to owners as they spend. Issuance journals at most 50 recipients inline so a platform-wide request remains bounded; the response reports activated and pending counts, and pending rows remain unspendable until the reconcile sweep appends their issuance ledger entries.
...
| `PLATFORM_SERVICE_RATE_LIMIT_PER_SECOND` | `2` | Sustained requests/second per user for each platform-credentialed service; `0` disables the limiter. |
