...
| `DELEGATION_SESSION_MAX_SECS` | `3600` (1 hour) | Absolute lifetime of a refreshable proxy-injected service-delegation session, fixed at the first mint and never extended by refresh. Values are clamped to `300`-`86400` seconds with a startup warning. Each bearer remains limited to 300 seconds. |
