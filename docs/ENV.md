...
| `JWT_ASSISTANT_FORWARD_TTL_SECS` | `300` (5 min) | **LEGACY / tombstone (cut 4, superseded by cut 5).** Was the TTL of the retired `assistant_forward` marker token. The assistant bridge now forwards a standard **delegated** access token (lifetime `MCP_DELEGATION_TOKEN_TTL_SECS`, 300s), so setting this env var no longer changes any live assistant token. Retained for one deploy while pre-migration marker tokens expire; removed with the rest of the marker machinery afterward. See CHAT_ASSISTANT_SPECS cut 5. |
