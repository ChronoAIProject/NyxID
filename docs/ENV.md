...
| `DELEGATION_SESSION_MAX_SECS` | `3600` (1 hour) | Absolute lifetime of a refreshable proxy-injected service-delegation session, fixed at the first mint and never extended by refresh. Values are clamped to `300`-`86400` seconds with a startup warning. A present but unparsable value also warns and uses the default. Each bearer remains limited to 300 seconds. |
| `DELEGATION_REFRESH_RATE_LIMIT_PER_SECOND` | `1` | Sustained service-delegation refreshes per second for each `(user_id, user_service_id)` pair. `0` disables the dedicated limiter. A present but unparsable value logs a startup warning and uses the default. |
| `DELEGATION_REFRESH_RATE_LIMIT_BURST` | `10` | Burst capacity for each service-delegation refresh bucket. A present but unparsable value logs a startup warning and uses the default. |
