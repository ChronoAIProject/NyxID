...
For initial NyxID setup, follow [Step 0 of the n8n quickstart](n8n.md#step-0--get-nyxid-running-and-create-an-agent-key).
...
In the web app, open **AI Services → Add Service**, select **Supabase Data API**,
choose **Direct** routing, and enter your **Supabase Project URL** and
**Supabase API Key**. The project URL field starts empty so you can enter your
own project. You can connect additional projects as separate services.
...
therefore exposes this connection through its generic proxy MCP tool. The agent
supplies a method and a path such as `todos?select=id,title&done=eq.false`.
...
For node routing, configure the full Data API URL ending in `/rest/v1` in the
node-local credential store.
