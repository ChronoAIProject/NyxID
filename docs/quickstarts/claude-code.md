...
3. Select `OpenAI Personal`. Click `Save`.
...
Open `AI Services` → `Agent Keys` → `[your key]` and look at the `Usage` card: each key shows its own request log, scoped to its allowed services. The admin audit log under `Admin` → `Audit Log` offers the same data with filtering by API key.
