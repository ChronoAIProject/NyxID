...
The usual timestamp and replay checks apply. The version tag separates the signature domain from legacy HTTP and WebSocket signatures. Service identity, target ID, and origin cannot be changed without invalidating the signature. Non-target HTTP calls retain the existing signature and wire shape, including compatibility with older nodes. Upgrade participating nodes before enabling the temporary Drive/Workspace catalog activation gate. See `GOOGLE_WORKSPACE_MULTI_ORIGIN_ENABLED` in [ENV.md](ENV.md).
