...
`api-aurinko` is a normal owner-scoped catalog connection backed by an encrypted account bearer credential. Existing active-service, agent-binding, scope, approval, and billing rules apply. The provider retains its `api_key` type for manual-token compatibility. Managed availability is derived from the named `aurinko_account_code` protocol and the current platform application configuration; it does not require replacing seeded provider rows or migrating existing credentials.

Managed onboarding reuses `OAuthState` and shared `UserEndpoint`/`UserApiKey`/`UserService` provisioning. The encrypted mailbox token has platform OAuth provenance and verified account identity. Managed channel bots reference the live same-owner key and resolve the current application webhook signing secret from platform configuration. They store no duplicate secrets. Disabling the AI Service blocks its linked bot's effects; deleting a bot retains the shared connection. Connection deletion rejects active bot consumers before changing the service or key. Manual AI Services and manual bots retain their independent encrypted account-token storage.

The narrow confidential-client exception documented in CLAUDE.md permits Aurinko's fixed authorization-code exchange without PKCE. It requires browser/session/actor/owner-bound single-use state, an exact registered callback, server-only Basic-authenticated exchange, configuration and credential-generation fences, and verified mailbox identity and permissions. Application credentials are never mailbox bearers. PKCE remains required for NyxID OAuth and supported upstream flows. Residual code-interception risk remains; these safeguards are not PKCE-equivalent. See [Aurinko integration](./AURINKO_INTEGRATION.md) for operator setup, provider support, lifecycle, and validation boundaries.
