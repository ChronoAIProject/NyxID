...
Managed Aurinko onboarding uses the platform application and signing secret. The mailbox connection is an owner-scoped `UserApiKey` shared with an AI Service; the bot stores only its connection ID. Google, Microsoft 365, Zoho Mail, hosted IMAP/SMTP, EWS and iCloud use the same hosted authorization path. Mail-server passwords and app passwords are entered only at Aurinko. POP3 is unsupported. Manual mailbox-token registration remains available.

Managed receive, send, setup, reconnect and deletion take a shared connection claim before bot lifecycle or ingress claims. AI Service Disable fences active bot work and blocks later mailbox token resolution. Delete of a connection used by an active bot returns an actionable conflict before modifying the service or key, including bots with pending or failed setup. Delete the bot first; it removes only that bot's subscription and retains the AI connection. Subscription cleanup can use the same verified credential after AI Service Disable. Reconnect replaces the encrypted token on the same key, increments its credential epoch, and never revokes the previous Aurinko account token automatically. A failed subscription setup returns the durable bot so Verify or retry can resume without duplicating it.
