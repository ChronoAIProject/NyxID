...
2. Add WhatsApp and Facebook Login for Business. Create an Embedded Signup configuration with a business integration system-user token, and allow the NyxID frontend domain and JavaScript SDK login. Enter App ID, App Secret, and Embedded Signup Configuration ID in NyxID.
...
`GET /api/v1/channel-bots/managed-onboarding/whatsapp` returns availability, public app/config IDs, the pinned Graph version, and supported feature types. Completion is `POST /api/v1/channel-bots/managed-onboarding/whatsapp/complete` with `{code, phone_number_id?, waba_id, business_id?, label, target_org_id?}`. Both require a human session or NyxID first-party user access token; API keys, delegated/service-account/relay identities and third-party OAuth clients are rejected. Completion and re-registration allow five attempts per user per minute using the shared database limiter. JSON completion returns 201; `Accept: text/event-stream` returns progress (`exchanging`, `subscribing`, `registering`) followed by a result or locally authored error.
...
Subscription/register failures remain visible on the saved bot. Retry a failed registration from its detail page. If subscription failed, the administrator can repair the WABA app subscription in Meta; deleting the incomplete bot and repeating Embedded Signup also repeats setup. A lost browser connection can leave a saved bot, so inspect the bot list before retrying signup. No Graph/database distributed transaction or durable setup job is claimed.

The platform callback is `GET/POST /api/v1/webhooks/channel/whatsapp/platform`. GET compares the generated platform Verify Token in constant time. POST acknowledges immediately, then verifies the raw `X-Hub-Signature-256` with the platform app secret and resolves each `entry[].changes[].value.metadata.phone_number_id` to an active or pending active bot. Each target uses the existing per-bot verification, phone filtering, dedup, routing, and dispatch path; unknown numbers are dropped. Per-WABA `override_callback_uri` and `verify_token` are attempted after app subscription, but override failure is non-fatal because the platform callback is sufficient. Overrides apply to every phone in a WABA, so a managed per-bot POST URL dispatches sibling numbers too. BYO per-bot routes remain unchanged. Background acknowledgment and concurrent-dedup limitations are the same as the existing WhatsApp receiver.
...
**Version deadline:** [Meta's versions page](https://developers.facebook.com/documentation/business-messaging/whatsapp/embedded-signup/versions) documents the requested `featureType` and `sessionInfoVersion: "3"` options under the legacy v2 flow. `sessionInfoVersion` selects the returned event payload; it does not select flow v3. This implementation preserves those requested options and the existing `GRAPH_API_VERSION` (`v25.0`). Meta explicitly deprecates v2 on **October 15, 2026**, lists v3 availability through October 2026, and recommends v4. Migrate the SDK launch contract and smoke-test a real Meta tenant before the legacy flow deadline. Meta documents granular targets as strings/numeric IDs and permits absent targets for all-assets grants; NyxID intentionally requires explicit WABA targets in both scopes. No stable numeric "already registered" error was documented, hence the independent connected-state check.
