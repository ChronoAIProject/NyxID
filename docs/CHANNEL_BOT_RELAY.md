...
Implement `PlatformAdapter` in `backend/src/services/channel_adapters/<platform>.rs`, then register its module and add it to `handlers/channel_bots.rs::resolve_adapter`, the single runtime registry.
...
- The public `/api/v1/webhooks/channel/{platform}/{bot_id}` GET/POST handlers use these hooks. No new per-platform handler or generic pipeline branch is needed. OpenClaw's descriptor disallows registration through this route because it has its own integration path.
...
5. Add a conversation route to an agent key with a callback URL, then send the business phone a message. `conversation_id` is the sender's WhatsApp ID (`from`, normally digits without `+`), and conversation type is always `private`. The bot remains `pending_webhook` until a verified POST arrives.
...
POST webhooks return 200 immediately and process in the background; verification/parse failures are suppressed with metadata-only logging. This is best-effort delivery: an acknowledged event can be lost on process failure. Meta can retry for seven days. The existing relay stores inbound platform message IDs but has **no inbound deduplication constraint**, so retries can cause repeated callbacks. No new replay collection or age cutoff is introduced; consumers should deduplicate by bot/conversation and platform message ID.
