...
- **Approvals runs on real data** (Calvin 2026-07-16, superseding the mock-store version): "Waiting on you" = real pending rows from `GET /approvals/requests?status=pending` rendered as `ApprovalCard` blocks (via `lib/assistant/approvals.ts` adapter: NyxID `rejected` → block `denied`, decide-channel `push` → `mobile`, card body from the shared `lib/approval-labels.ts` helpers); Approve/Deny posts `POST /approvals/requests/{id}/decide`. **History** = recent decided rows from the same listing. Live updates by polling (pending 5s, history 15s, focused-tab only — the web app has no push channel; Telegram/mobile push remain the real-time notifiers). Amber sidebar badge = server-side pending `total`, shared query cache with the view. The backend list API now exposes `expires_at`, so the card countdown is the request's real expiry. In-chat approval cards inside scripted demo conversations stay mock (fake request ids; deny semantics per the run-gate rules) until the API pass wires chat turns to real runs.
