...
labels cannot pass strict verification. Families parse generically as a one-to-three-digit
major and optional minor version, yielding `gpt_<major>[_<minor>][_pro]`.
...
control rather than trusting the clicked entry. The switcher also accepts exact
first-line tier entries “Pro”/“专业” when its trigger already shows the requested
family. It may open one submenu whose first line is exactly the requested
family token (e.g. “GPT-6”); nested discovery records a new menu-identity baseline.
It never clicks an unrecognized family, arbitrary navigation, or upgrade prose.
Prompt typing and sending
still require unobstructed, actionable controls and composer read-back.

Completed and failed results retain the requested `model_label` and add canonical
...
presence or task diagnostics. Before a strict failure, `selecting_model` acknowledges
`switcher=<meta> effort=<meta> reason=<selection reason>`. Failed results and
capacity reroutes retain both observations; worker `last_error` stays
`model_unavailable@selecting_model`, including a final pre-send read-back failure.
Local logs include raw labels only when explicitly
...
|---|---|---|
| `GET /task` | `?worker=worker_1&script_version=&instance_id=` | Idle response, or a task with retry counters, optional `dispatch_attempt_id`, prompt, attachment, conversation URL, and project hint. |
...
- **Worker capacity**: `usage_limit_reached` and `model_unavailable` exclude the
  ...
  must have `last_seen_at` within `WORKER_RECENT_SECS` (120 seconds), be outside
  the exclusions, and not be draining, logged out, unhealthy, or cooling down.
  Heartbeat, ack, and claim refresh presence: a worker busy on a long task
  remains eligible without polling for new tasks. Claim and terminal recheck
...
After Send, an idle empty/unextractable turn or scoped error banner reloads the conversation
...
infrastructure retries. A `wait` resume decision always re-enters the bounded
response wait, including a queued turn with no generation indicator yet.
Before Send, a generic ChatGPT error banner reloads the page once; a persistent
banner uses the DOM-shape failure path (one relaunch cycle, then
`browser_recovery_exhausted`). Pre-send capacity banners retain capacity classification.
Maximum generation timeout is `response_timeout` and
...
expires. Invalid or less-than-one values use the 900-second default and emit
one metadata-only startup log line. Presence exposes server-resolved
`cooldown_until`. Legacy `page_url` query/body fields are accepted but ignored;
every presence write removes any previously stored URL.
...
|---|---|
| `button[data-testid="model-switcher-dropdown-button"]`; `header button[aria-haspopup="menu"]` | Verified by test: generic family read-back, full-label and tier-only selection, one family submenu, wrong-family/prose rejection, sidebar isolation |
| `header button[aria-haspopup="listbox"]`; `[role="banner"] button[aria-haspopup]` | Assumed fallback; no dedicated fixture |
| `button.__composer-pill[aria-haspopup="menu"]`; composer-local menu buttons | Verified by test: Extended preference, explicit Standard, unrecognized Tools/attach controls, strict evidence, cancellation, deadlines |
| `[role="menu"]`, `[role="listbox"]`, `[role="menuitemradio"]` | Verified by test: menu identity, nested selection, stale menus and read-back |
| `[role="menuitem"]` | Verified by test: tier selection in a family submenu |
| Selectable `[role="option"]`; `composer-intelligence-pro-thinking-effort-trigger` | Assumed fallback; option exclusion is tested, selection through these variants is not |
...
Startup idempotently rewrites exactly the retired seeded pool default
`chatgpt-5.5-pro` to `chatgpt-6-pro`; custom labels are preserved. It logs the
updated pool count once, without audit events. Explicit per-task GPT-5.5 requests
remain supported by generic family parsing when the account offers that family.
