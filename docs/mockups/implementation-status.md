...
| Permission picker | One access-selection component searches NyxID scopes, service permissions, and connections during key creation, with separate selected panels, counts, and service icons. Account-only keys need no service selection. The same final access-review component is used for existing and new keys, with an outcome badge. Runtime inventory comes from options/catalog APIs; no mockup account data ships in production. |
