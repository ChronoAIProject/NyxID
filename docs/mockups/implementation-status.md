...
| Permission picker | One search for permissions and connections during key creation, separate selected panels with counts and service icons, mixed group selection, keyboard dismissal and focus retention. Account-only keys need no service selection. Runtime inventory comes from options/catalog APIs; no mockup account data ships in production. |
