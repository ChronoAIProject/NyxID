...
## 19. In-app design gallery — removed

A dev-only `/design/action-cards` gallery route (real components + seeded fixtures)
existed during design review and was removed on 2026-07-30 after sign-off, per
prefer-deletion. The living demo surface is `/assistant?mock` (Part I §10); the
target-design catalogue remains §17 + the static showcase (§18).
