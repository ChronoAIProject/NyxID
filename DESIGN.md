...
| 2026-07-31 | Pre-answer loader is a four-ball Newton's cradle | Replaces the five-dot "domino" loader. **Four** 6px balls on an 8px pitch — a 30px pack with a 2px seam, where the old five 4px dots sat on the same pitch with a full ball-width of air between them, so nothing read as being in contact and the impulse crossed open space. Balls 1 and 4 are strikers; **2 and 3 are ballast and never move or change** — a cradle's defining property is that its middle does nothing, so animating it argues against the mechanism (the old ±2px wiggle on the inner dots was the loudest tell this was decoration, and a later attempt to light them in sequence failed too: 22ms apart is 1.3 frames at 60Hz, so it lands as one central blink and reads as typing dots). Motion derives from mass: a 7px swing, nearly a full pitch, opening the seam to 9px; gravity easing that is asymmetric and time-reversed between the fall (`cubic-bezier(0.4, 0, 0.7, 0.4)`) and the climb (`cubic-bezier(0.3, 0.6, 0.6, 1)`); a 462ms swing where the old impulses were 72ms — four frames at 60Hz, below the window where an eye reads a trajectory at all; a short 5% apex dwell; and opacity tracking kinetic energy, dimmest at the apex and brightest at the collision, handed over inside a 66ms window so **exactly one striker is lit at a time** — light is transferred, not duplicated. No vertical rise: a pixel is sub-perceptual and anything larger needs an arm twice the loader's height and interpolates as a straight chord, reading as a hop. Period 2200ms, one impact every 1100ms. Colour moves from `text-tertiary` at a flat 70% (~1.8:1 dark, effectively invisible) to `muted-foreground` cycling 65%→100%, so a resting ball clears 3:1 in **both** themes (3.8:1 dark, 3.2:1 light). The exit sweep interpolates from each ball's captured live transform and opacity (`--domino-exit-x` / `--domino-exit-opacity`, written by `StreamingDots`, with `animation-fill-mode: both` covering the stagger delay); a `to`-only exit would start from the underlying undisplaced style and snap a displaced striker back into the pack for a frame before throwing it. Reduced motion holds the balls still and breathes them together at a narrower 65%→85%, rather than freezing them — a frozen loader reads as a dead turn. |
