...
For Ornn calls, use only ordinary HTTP `/api/v1/proxy/{grant.ornn_proxy_service_id}/...` with the separately attached SA Ornn credential and the dedicated endpoint's explicit `proxy_operation_policy` (exact skill reads, POST upload, and PUT version publication only). The slug forms, `/keys`, generic `/catalog`, provider self-management, and spec/routing changes described below are not available to Curation tokens. Do not use the creator's credentials or ask for broad Ornn admin authority to bypass a denied request.

NyxID recommendation management does not edit package bytes. The companion Ornn contract ([#1247](https://github.com/ChronoAIProject/Ornn/issues/1247)) uses only `ornn:skill:read` + `ornn:skill:publish`, with exact object write grants to the SA UUID. Upload new readable packages with `POST /api/v1/skills?public=true`, update content with raw ZIP `PUT /api/v1/skills/{id}`, and omit `isPrivate` entirely on updates. Curation must not use the binding/privacy mutations in the general flow below; use the NyxID recommendation API instead. See [the separate identity recipe](../../docs/SERVICE_ACCOUNTS.md#separate-ornn-identity-and-endpoint). Actual package create/read/update must use that deployed Ornn content API; retain immutable versions and deny package/history deletion. Load the deployed Ornn contract and verify the resource/action boundary before claiming content CRU is delivered. Names are advisory; supplied exact refs are pins, not evidence of content approval. Consumers see changed defaults on their next fetch; installed copies require their own update, and restoring instructions cannot undo past external effects.
