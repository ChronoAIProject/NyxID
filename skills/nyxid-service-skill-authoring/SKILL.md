...
For Ornn calls, use only ordinary HTTP `/api/v1/proxy/{grant.ornn_proxy_service_id}/...` with the separately attached SA Ornn credential and the dedicated endpoint's explicit `proxy_operation_policy` (exact skill reads, creation, content/visibility updates, and creator settings). The slug forms, `/keys`, generic `/catalog`, provider self-management, and spec/routing changes described below are not available to Curation tokens. Do not use the creator's credentials or ask for broad Ornn admin authority to bypass a denied request.
