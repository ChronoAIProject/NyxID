...
- Exact count and page queries now execute concurrently, alongside event-type option retrieval. Empty counts use the existing `_id` index rather than fetching every document. Totals remain exact; no estimated count or capped pagination was introduced.
...
| `audit.default.count` | 500,000 | 0 | 97 | 0 | 500,000 | 55 |
| `audit.default.find` | 50 | 50 | 0 | 50 | 50 | 0 |
| `audit.event_type.count` | 0 | 4,167 | 1 | 0 | 4,167 | 0 |
| `audit.event_type.find` | 50 | 50 | 0 | 50 | 50 | 0 |
| `audit.search.count` | 0 | 3,500,000 | 1,723 | 500,000 | 0 | 581 |
| `audit.search.find` | 10,202 | 10,202 | 221 | 10,202 | 10,202 | 15 |
| `audit.date.count` | 0 | 14,505 | 2 | 0 | 14,505 | 2 |
| `audit.date.find` | 50 | 50 | 0 | 50 | 50 | 0 |
| `audit.search_date.count` | 14,505 | 14,505 | 104 | 14,505 | 14,505 | 100 |
| `audit.search_date.find` | 10,202 | 10,202 | 63 | 10,202 | 10,202 | 62 |
| `audit.status.count` | 0 | 50,000 | 9 | 0 | 50,000 | 7 |
| `audit.status.find` | 498 | 498 | 0 | 498 | 498 | 0 |
| `usage.summary` | 200,000 | 180,000 | 298 | 2,831 | 2,548 | 74 |
| `usage.ranking` | 200,000 | 180,000 | 288 | 2,831 | 2,548 | 84 |

- Unfiltered audit counts become index-only scans of `_id_`; the page remains a
  50-key/50-document sort-index scan. Exact totals are preserved.
- Global search counts change from seven unbounded index scans to one collection
  pass (1,723 → 581 ms). The page keeps its sort-index scan and avoids plan
  competition (221 → 15 ms). No regex or pagination semantics change.
- Event-type, date and status queries retain their existing usable indexes.
  Search-plus-date uses a date-bounded IXSCAN followed by regex FETCH (14,505
...
Event-type cache measurement: cold **1.648 ms**, warm **0.004375 ms**,
